/*
 * gem_vdi.c - native GEM VDI raster core for ELKS.
 *
 * Copyright 1999 Caldera Thin Clients, Inc.
 * Copyright 1987 Digital Research, Inc.
 *
 * in-process VDI drawing over the GPL FreeGEM PC screen driver
 * state is one foreground, one background, one writing mode, one inclusive
 * clip, coords are clipped before a driver so video offsets fit their
 * 16-bit segment
 */
...
{
	/* drawing goes straight to PC video memory, nothing to flush */
	(void) screen;
}
	...
	GEM_VDI_UWORD pattern, GEM_VDI_WORD draw_last)
{
	...
	/* a solid pattern takes the drivers fast scan-line path */
	if (pattern == 0xffffU) {
...
/*
 * one clipped pattern span, the pattern row and mask bit come from absolute
 * y and x so separate spans land on the same screen-anchored 8-pixel grid
 * and tile
 */
static void
gem_pattern_span(GEM_VDI_SCREEN *screen, GEM_VDI_COORD x1,
	GEM_VDI_COORD x2, GEM_VDI_COORD y, const GEM_VDI_UBYTE *pattern)
{
	GEM_VDI_COORD x;
	GEM_VDI_COORD run_x;
	GEM_VDI_WORD in_run;
	...
	if (gem_mode == GEM_VDI_REPLACE) {
		screen->driver->fill_pattern(x1, y, x2, y,
			gem_driver_color(screen, gem_foreground),
	...
	for (x = x1; x <= x2; x++) {
		if (source & mask) {
			...
			screen->driver->horizontal_line(run_x, x - 1, y,
	...
	if (in_run)
		screen->driver->horizontal_line(run_x, x2, y,
			gem_driver_color(screen, gem_foreground), gem_mode);
}

void
gem_vdi_fill_pattern(GEM_VDI_SCREEN *screen, GEM_VDI_COORD x,
	GEM_VDI_COORD y, GEM_VDI_COORD width,
	GEM_VDI_COORD height, const GEM_VDI_UBYTE *pattern)
{
	GEM_VDI_COORD x2;
	GEM_VDI_COORD y2;
	GEM_VDI_COORD row;
	...
	if (x > x2 || y > y2)
		return;
	/* non-replace modes keep the pixels under clear pattern bits, so they
	 * go row by row instead of through the opaque driver fill */
	restore_cursor = gem_begin_draw(x, y, x2, y2);
	if (gem_mode == GEM_VDI_REPLACE) {
		...
		for (row = y; row <= y2; row++)
			gem_pattern_span(screen, x, x2, row, pattern);
	}
	gem_end_draw(restore_cursor);
}
...
gem_vdi_bitmap(GEM_VDI_SCREEN *screen, GEM_VDI_COORD x,
	GEM_VDI_COORD y, GEM_VDI_COORD width,
	GEM_VDI_COORD height, const GEM_VDI_BITS *bits)
{
	GEM_VDI_COORD row;
	...
	/* clip before the drivers replace-mode path, source_x and the row
	 * stride let it read the RSC/ICN words in place */
	x1 = x;
	y1 = y;
	x2 = x + width - 1;
	y2 = y + height - 1;
	if (gem_mode == GEM_VDI_REPLACE && screen->driver->bitmap_replace) {
		...
		screen->driver->bitmap_replace(x1, y1,
			x2, y2, row_bits, source_x,
			words_per_row,
			gem_driver_color(screen,
				gem_foreground),
			gem_driver_color(screen,
				gem_background), gem_use_background);
		gem_end_draw(restore_cursor);
		return;
	}
	...
	for (row = 0; row < height; row++) {
		for (column = 0; column < width; column++) {
			...
			if (word & mask)
				...
				continue;
			gem_raw_point(screen, x + column, y + row, color,
				gem_mode);
		}
		row_bits += words_per_row;
	}
...
gem_vdi_glyph(GEM_VDI_SCREEN *screen, GEM_VDI_COORD x,
	GEM_VDI_COORD y, GEM_VDI_COORD width,
	GEM_VDI_COORD height, const GEM_VDI_UBYTE *rows,
	GEM_VDI_UBYTE source_left_bit)
{
	GEM_VDI_COORD x1;
	GEM_VDI_COORD y1;
	GEM_VDI_COORD x2;
	...
	GEM_VDI_WORD restore_cursor;
	/* one source row is exactly one byte, wider shapes are rejected */
	if (!screen || !rows || !source_left_bit || width <= 0
		|| width > 8 || height <= 0)
		return;
	x1 = x;
	y1 = y;
	...
	if (gem_mode == GEM_VDI_REPLACE && screen->driver->glyph_replace) {
		screen->driver->glyph_replace(x1, y1, x2, y2,
			row_bits, source_x,
			source_left_bit,
			gem_driver_color(screen, gem_foreground));
		gem_end_draw(restore_cursor);
	...
	for (screen_y = y1; screen_y <= y2; screen_y++) {
		...
		for (screen_x = x1; screen_x <= x2; screen_x++) {
			if (*row_bits & source_bit)
				screen->driver->write_pixel(screen_x,
					screen_y,
					gem_driver_color(screen,
						gem_foreground), gem_mode);
			source_bit >>= 1;
...
void
gem_vdi_form(GEM_VDI_SCREEN *screen, GEM_VDI_COORD x,
	GEM_VDI_COORD y, GEM_VDI_COORD width,
	GEM_VDI_COORD height, const GEM_VDI_UBYTE *rows, GEM_VDI_UWORD stride)
{
	GEM_VDI_COORD x1;
	GEM_VDI_COORD y1;
	GEM_VDI_COORD x2;
	...
	const GEM_VDI_UBYTE *row_bits;
	GEM_VDI_WORD restore_cursor;
	if (!screen || !rows || !stride || width <= 0 || height <= 0)
		return;
	if (width > (GEM_VDI_COORD) (stride * 8U))
		width = (GEM_VDI_COORD) (stride * 8U);
	x1 = x;
	y1 = y;
	x2 = x + width - 1;
	y2 = y + height - 1;
	...
	if (x1 > x2 || y1 > y2)
		return;
	color = gem_driver_color(screen, gem_foreground);
	row_bits = rows + (GEM_VDI_UWORD) (y1 - y) * stride;
	restore_cursor = gem_begin_draw(x1, y1, x2, y2);
	for (screen_y = y1; screen_y <= y2; screen_y++) {
		column = (GEM_VDI_UWORD) (x1 - x);
		for (screen_x = x1; screen_x <= x2; screen_x++) {
			if (row_bits[column >> 3]
				& (GEM_VDI_UBYTE) (0x80U >> (column & 7U)))
				screen->driver->write_pixel(screen_x,
					screen_y, color, gem_mode);
			column++;
		}
		row_bits += stride;
	}
	...
	GEM_VDI_UWORD font_segment, GEM_VDI_UWORD font_offset)
{
	...
	GEM_VDI_WORD restore_cursor;
	GEM_VDI_WORD result;
	/* fast path only, anything needing source clipping or conversion goes
	 * to gem_vdi_glyph(), count is capped by the 80-entry VDI intin array,
	 * stride is bytes or words */
	if (!screen || !screen->driver
		|| !screen->driver->text_replace || !characters
		|| !font_segment || !count || count > 80U
		|| (stride != 1U && stride != 2U) || x < 0
		|| y < 0 || gem_mode != GEM_VDI_REPLACE
		|| ((GEM_VDI_UWORD) x & 7U)
		|| x > screen->xres - 8 || y > screen->yres - 16)
		return 0;
	/* repeated adds stop ia16-gcc emitting a MUL for count*8 */
	x2 = x;
	remaining = count;
	while (remaining--) {
		if (x2 > screen->xres - 8)
	...
	if (x < gem_clip_x1 || y < gem_clip_y1
		...
		return 0;
	restore_cursor = gem_begin_draw(x, y, x2, y2);
	result = screen->driver->text_replace(x, y, characters,
		count, stride, font_segment,
		font_offset, gem_driver_color(screen, gem_foreground));
	gem_end_draw(restore_cursor);
	return result;
}
...
{
	/* negate in unsigned maths so -32768 comes out as exactly 32768 */
	return value < 0 ? (GEM_VDI_UWORD) (0u - (GEM_VDI_UWORD) value)
		: (GEM_VDI_UWORD) value;
}

void
gem_vdi_fill_polygon_pattern(GEM_VDI_SCREEN *screen,
	...
	const GEM_VDI_UBYTE *pattern)
{
	...
	for (point = 1; point <= count; point++) {
		...
		/* GEM_POLYGON_EDGE is 14 bytes, indexing by edge_count makes
		 * ia16-gcc emit a MUL, so step a pointer instead */
		edge_write->y_min = y1;
...
next_edge:
	...
	if (!edge_count)
		return;
	restore_cursor = gem_begin_draw(gem_clip_x1, min_y, gem_clip_x2, max_y);
		...
		if (y >= gem_clip_y1 && y <= gem_clip_y2) {
			for (i = 0; i + 1 < intersections; i += 2) {
				...
				if (x1 > x2)
					continue;
				if (pattern)
					gem_pattern_span(screen, x1, x2,
						y, pattern);
				else
					screen->driver->horizontal_line(x1, x2,
						y,
						gem_driver_color
						(screen, gem_foreground),
						gem_mode);
			}
		}
		edge_read = gem_polygon_edges;
...
{
	gem_vdi_fill_polygon_pattern(screen, count, xy, 0);
}

/*
 * multiply by a Q7 factor as two 16-bit halves, the initial 64 rounds the
 * final 7-bit shift, overflow sticks at 32767
 */
...
gem_arc_y(GEM_VDI_COORD radius, GEM_VDI_WORD angle)
{
	...
	if (normalized >= 1800)
		value = -value;
	/* screen Y grows downward, opposite of GEM's mathematical Y */
	return -value;
}
	...
	GEM_VDI_WORD rule)
{
	...
	case 2:
		return source & (GEM_VDI_COLOR) ~destination & mask;
	case 3:
		return source;
	case 4:
		return destination & (GEM_VDI_COLOR) ~source & mask;
	case 5:
		return destination;
	case 6:
		return source ^ destination;
	case 7:
		return source | destination;
	case 8:
		return (GEM_VDI_COLOR) ~(source | destination) & mask;
	case 9:
		return (GEM_VDI_COLOR) ~(source ^ destination) & mask;
	case 10:
		return (GEM_VDI_COLOR) ~destination & mask;
	case 11:
		return source | ((GEM_VDI_COLOR) ~destination & mask);
	case 12:
		return (GEM_VDI_COLOR) ~source & mask;
	case 13:
		return ((GEM_VDI_COLOR) ~source & mask) | destination;
	case 14:
		return (GEM_VDI_COLOR) ~(source & destination) & mask;
	default:
		return mask;
...
gem_vdi_blit(GEM_VDI_SCREEN *screen, GEM_VDI_COORD dst_x,
	GEM_VDI_COORD dst_y, GEM_VDI_COORD width,
	GEM_VDI_COORD height, GEM_VDI_COORD src_x,
	GEM_VDI_COORD src_y, GEM_VDI_WORD rule)
{
	GEM_VDI_COORD row;
	GEM_VDI_COORD column;
	...
	if (width <= 0 || height <= 0)
		return;
	restore_cursor =
		gem_begin_draw(dst_x, dst_y, dst_x + width - 1,
		dst_y + height - 1);
	/* rule 3 is a plain copy, the PC driver moves video bytes on all planes
	 * and handles overlap, bounds checked with subtraction so a 16-bit coord
	 * sum cant wrap */
	if ((rule & 15) == 3
		&& src_x >= 0 && src_y >= 0
		...
		&& gem_pc_screen_blit(dst_x, dst_y, width,
			height, src_x, src_y)) {
		gem_end_draw(restore_cursor);
		return;
	}
	...
	for (row = row_start; row != row_end; row += row_step) {
		for (column = column_start;
			column != column_end; column += column_step) {
			if (!gem_inside_clip(dst_x + column, dst_y + row))
				continue;
			source = screen->driver->read_pixel(src_x + column,
				src_y + row);
			destination =
				screen->driver->read_pixel(dst_x + column,
				dst_y + row);
			screen->driver->write_pixel(dst_x + column,
				dst_y + row,
				gem_driver_color(screen,
					gem_blit_rule
					(source, destination,
						rule)), GEM_VDI_REPLACE);
		}
	}
	gem_end_draw(restore_cursor);
}
...
{
	GEM_VDI_WORD redraw;
	/* a serial packet may change only the buttons, skip the redraw */
