...
/*
 * gem_irc_protocol.c - fixed-buffer IRC line engine
 *
 * two callbacks, one complete outbound IRC line and one parsed UI event,
 * the transport owns the socket and the front end owns the window loop
 */
...
gem_irc_equal(const char *left, const char *right)
{
	...
	while (*left && *right) {
		if (*left != *right
			&& gem_irc_fold_char(*left) !=
			gem_irc_fold_char(*right))
			return 0;
		left++;
		right++;
	}
...
gem_irc_starts_with(const char *text, const char *word)
{
	...
	while (*word) {
		if (!*text
			|| gem_irc_fold_char(*text) != gem_irc_fold_char(*word))
			return 0;
		text++;
		word++;
	}
...
/*
 * store an accepted nick, an over-long server-supplied nick still shows
 * up in its event but never truncates the current-nick state
 */
static GEM_IRC_WORD
...
static GEM_IRC_WORD
gem_irc_append_char(GEM_IRC_CLIENT *client, GEM_IRC_UWORD *length, char value)
{
	if (!client || !length)
		return GEM_IRC_BAD_ARGUMENT;
...
/* append user text, CR and LF refused so no command injection */
static GEM_IRC_WORD
...
/* middle params may not hold control bytes, spaces, or a colon */
static GEM_IRC_WORD
...
/* add CR/LF after at most 510 payload bytes, byte 512 holds the terminator */
static GEM_IRC_WORD
	...
	const char *const *parameters, GEM_IRC_UWORD count)
{
	...
	if (result == GEM_IRC_OK)
		result = gem_irc_add_trailing(client, &length,
			parameters[count - 1U]);
	if (result != GEM_IRC_OK)
		return result;
	return gem_irc_finish(client, length);
...
{
	/* an event callback must not reset the client mid-parse */
