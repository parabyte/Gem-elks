...
/*
 * The resident AES discovers this fixed client tree by its final LASTOB bit.
 * Item records may otherwise replace all flags while a directory view is
 * rebuilt.  Preserve the terminator only on the final absolute OBJECT index;
 * all values are unscaled 16-bit GEM words.
 */
#define DESK_ITEM_FLAGS(obj, flags) \
	((UWORD)(((UWORD)(flags) & (UWORD)~LASTOB) \
		 | ((obj) == (NUM_SOBS - 1) ? LASTOB : 0)))
