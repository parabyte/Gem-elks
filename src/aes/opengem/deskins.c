...
VOID ins_posdisk(WORD dx, WORD dy, WORD *pdx, WORD *pdy)
{
	...
	for(pa=G.g_ahead; pa; pa=pa->a_next)
	{
	  if (pa->a_type == AT_ISDISK || pa->a_type == AT_ISTRSH ||
	      pa->a_type == AT_ISGRP)
	  {
	    ax[na] = desk_word_divide(pa->a_xspot - G.g_xdesk,
					 G.g_icw, NULL);
	    ay[na] = desk_word_divide(pa->a_yspot - G.g_ydesk,
					 G.g_ich, NULL);
	    ++na;
	  }
	...
	{
	  ydir = -1;
	  ydiff = (WORD)(0U - (UWORD)ydiff);
	}
	else
	  ydir = 1;
	xstep = xdir < 0 ? -G.g_icw : G.g_icw;
	ystep = ydir < 0 ? -G.g_ich : G.g_ich;
...
/* If we've gone off the screen, try going the other way */
	  if (*pdy >= G.g_ydesk + G.g_hdesk) *pdy = dy - ystep;
	}
	else
	{
	  *pdx = dx + xstep;
	  *pdy = dy;
/* If we've gone off the screen, try going the other way */
	  if (*pdx >= G.g_xdesk + G.g_wdesk) *pdx = dx - xstep;
	}
 ...
 * new home for it. Set dx and dy to tend toward the centre of the screen. */
	if (*pdx >= G.g_xdesk + desk_word_divide(G.g_wdesk, 2, NULL))
	  dx = -G.g_icw;
	else					 dx =  G.g_icw;
	if (*pdy >= G.g_ydesk + desk_word_divide(G.g_ydesk, 2, NULL))
	  dy = -G.g_ich;
	else					 dy =  G.g_ich;

	nx = desk_word_divide(pdx[0] - G.g_xdesk, G.g_icw, NULL);
	ny = desk_word_divide(pdy[0] - G.g_ydesk, G.g_ich, NULL);
	for (n = 0; n < na; n++)
	{
...
{			       
	...
	BYTE		cletter[2], clabel[13], nlabel[13];
	...
	change = FALSE;
#if defined(ELKS) && ELKS
	/* The mounted namespace is identified by its real absolute path. */
	cletter[0] = '/';
#else
...
#if defined(ELKS) && ELKS
	/* DRREM is the useful Defaults operation in the ELKS overlay. */
	tree[DRREM].ob_state &= ~DISABLED;
#else
...
#if defined(ELKS) && ELKS
	if (tree[DRINST].ob_state & SELECTED) /* Apply root-icon changes. */
	{
	  if (pa->a_aicon != icon)
	  {
	    pa->a_aicon = icon;
	...
	else if (tree[DRREM].ob_state & SELECTED) /* Restore useful defaults. */
	{
	  ...
	  if (strcmp(pa->a_pappl, "POSIX /"))
	  {
	    scan_str("POSIX /@", &pa->a_pappl);
	    change = TRUE;
	  }
	}
#else
			   ...
			   ICONBLK *pic, BYTE *ptext)
{
	...
	memcpy(pic, desk_byte_index((BYTE *)&G.g_iblist[0], (UWORD)nicon,
				    (UWORD)sizeof(ICONBLK)), sizeof(*pic));
	pic->ib_ptext = gem_near_pointer_words(ptext);
	object = desk_object_at(tree, obj);
	object->ob_type = G_ICON;
	object->ob_spec = gem_near_pointer_words(pic);
}
...
VOID insa_elev(LPTREE tree, WORD nicon, WORD numics)
{
	...
	if ( numics > 1)
	{
	  ...
	  if (h < (desk_word_divide(gl_hbox, 2, NULL) + 2))
	    h = desk_word_divide(gl_hbox, 2, NULL) + 2;
	...
	lp = G.a_datastart
		+ *((UWORD *)(G.a_datastart + gl_iconstart + nicon * 2));
#if defined(ELKS) && ELKS
	/* Named icon 27 is presented with the runtime tool bitmap as Shell. */
	if (nicon == 27)
	  strlcpy(&gl_lngstr[0], "Shell", sizeof(gl_lngstr));
	else
#endif
	...
	LPBYTE	st_ad;
{
	...
	while ((ch = *st_ad) != '\0')
	{
	  /* Decimal accumulation modulo 65,536 using only word additions. */
	  twice = retval + retval;
	  eightfold = twice + twice;
	  eightfold += eightfold;
	  retval = eightfold + twice + (UWORD)(ch - '0');
	  st_ad++;
	}
	return((WORD)retval);
}
...
/* */
	...
	desk_object_at(tree, field)->ob_state |= SELECTED;
...
#if MULTIAPP
	{
		...
		slot.words = tree[APMEMSZ].ob_spec;
		tedi = slot.tedinfo;
		if (tedi) {
			slot.words = tedi->te_ptext;
			text = slot.bytes;
		} else {
			text = (GEM_SLOT_BYTE_POINTER) 0;
		}
		if (text) {
			lstlcpy(&memszstr[0], text, sizeof(memszstr));
			nmemsz = ins_latoi(&memszstr[0]);
		} else {
			nmemsz = omemsz;
		}
	}
	...
	if (field == 2)
	  nflag |= AF_ISPARM;
	desk_object_at(tree, APGEM + field)->ob_state &= ~SELECTED;
...
VOID ob_actxywh(LPTREE tree, WORD obj, GRECT *p)
{
	...
	p->g_w = object->ob_width;
	p->g_h = object->ob_height;
} /* ob_actxywh */
...
VOID ob_relxywh(LPTREE tree, WORD obj, GRECT *prect)
{
	LPOBJ object;
				...
				/* get x,y,w,h for specified object	*/
	object = desk_object_at(tree, obj);
	LWCOPY(ADDR(prect), (LPVOID)&object->ob_x, 4);
} /* ob_relxywh */
