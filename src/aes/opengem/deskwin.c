...
/* round work rect to character cell size */				
    work->g_w = desk_word_multiply(
		desk_word_divide(work->g_w, gl_wchar, NULL), gl_wchar);
    work->g_h = desk_word_multiply(
		desk_word_divide(work->g_h, gl_hchar, NULL), gl_hchar);
...
VOID win_view(WORD vtype, WORD isort)
{
	...
	G.g_incol = desk_word_divide(G.g_wfull - gl_wchar,
				      G.g_iwspc, NULL);
}
...
// DESKTOP v2.x	  pw->w_obid = 0;
	  pw->w_pncol = desk_word_divide(pt->g_w - gl_wchar,
					  G.g_iwspc, NULL);
	  pw->w_pnrow = desk_word_divide(pt->g_h - gl_hchar,
					  G.g_ihspc, NULL);
	  pw->w_vncol = 0x0;
	  pw->w_vnrow = 0x0;
/* BugFix	*/
...
{
	FNODE		*pf;
	WORD		cnt, col_count, row_count;
	WORD		skip_cols, skip_rows, w_space;
						/* zero out obid ptrs	*/
						/*   in flist and count	*/
						/*   up # of files in	*/
						/*   virtual file space	*/
	...
	/*
	 * Walk complete virtual rows and then the partial column.  This is the
	 * same offset as cvrow * incol + cvcol, but the linked-list walk itself
	 * performs the scaling and avoids a separate 16-bit multiplication.
	 */
	skip_rows = pwin->w_cvrow;
	while ((skip_rows--) && pf)
	{
	  skip_cols = G.g_incol;
	  while ((skip_cols--) && pf)
...
VOID win_bldview(WNODE *pwin, WORD x, WORD y, WORD w, WORD h)
{
	...
	obj_wfree(pwin->w_root, x, y, w, h);
						...
						/*   1st file in current*/
						/*   view		*/
	win_ocalc(pwin, desk_word_divide(w, G.g_iwspc, NULL),
		  desk_word_divide(h, G.g_ihspc, NULL), &pstart);
...
*/
	  ...
	  pstart->f_obid = obid;
						/* build object		*/
	  object = desk_object_at(G.g_screen, obid);
	  object->ob_state = WHITEBAK /* Not in DESKTOP v1.2 | DRAW3D */;
	  object->ob_flags = DESK_ITEM_FLAGS(obid, NONE);
	  switch(G.g_iview)
	  {
	    case V_TEXT:
		/*
		 * The original dr_fnode() obtains this exact display line from
		 * ob_sfcb().  A resident AES cannot execute dr_code() in gemdesk's
		 * process, so retain the original formatter and expose its completed
		 * bytes as GEM's native G_STRING object.  Formatting happens only when
		 * a view is rebuilt; redraw, selection, dragging, and hit testing then
		 * remain inside the ordinary resident object path.
		 *
		 * ob_sfcb() returns 45 bytes.  Clamp defensively before writing the
		 * sole NUL byte; every index and length is one 16-bit word and the
		 * fixed union in deskvars.h supplies the storage without allocation.
		 */
		text_length = ob_sfcb((LPBYTE) &pstart->f_junk,
				      &gl_text_rows[obid][0]);
		if (text_length < 0
		    || text_length >= DESK_TEXT_ROW_BYTES)
		  text_length = DESK_TEXT_ROW_BYTES - 1;
		gl_text_rows[obid][text_length] = 0;
		object->ob_type = (object->ob_type & 0xff00) | G_STRING;
		object->ob_spec =
			gem_near_pointer_words(&gl_text_rows[obid][0]);
		win_icalc(pstart);
		break;
	    case V_ICON:
		...
		object->ob_spec =
			gem_near_pointer_words(&gl_icons[obid]);
		/*movs(sizeof(ICONBLK), &G.g_iblist[i_index], &gl_icons[obid]);*/
		memcpy(&gl_icons[obid], &G.g_iblist[i_index], sizeof(ICONBLK));
		gl_icons[obid].ib_ptext =
			gem_near_pointer_words(&pstart->f_name[0]);
		/*
		 * Center the icon in its cell, as app_blddesk does for the
		 * desktop icons.  app_rdicon widens the label box to
		 * ib_wtext = 12 * gl_wschar (the cell width), but the ICN's
		 * ib_xicon centers the icon only in its original narrower
		 * small-font box; without this the label, centered in the
		 * wide box, sits right of the icon.
		 */
		gl_icons[obid].ib_xicon = desk_word_divide(
			G.g_wicon - gl_icons[obid].ib_wicon, 2, NULL);
...
VOID win_blt(WNODE *pw, BOOLEAN vertical, WORD newcv)
{
	...
	if ( rc_equal(&c, &t) )
	{
	  ...
	  if (pn > delcv)
	  {
		...
		{
	    sx = dx = 0;
	    sy = desk_word_multiply(delcv, G.g_ihspc);
	    dy = 0;
	    wblt = c.g_w;
	    hblt = c.g_h - sy;
		}
		...
		{
			sy = dy = 0;
			sx = desk_word_multiply(delcv, G.g_iwspc);
			dx = 0;
			wblt = c.g_w - sx;
			hblt = c.g_h; 
		}
...
win_shwall()
{
	...
	for(ii = 0; ii < NUM_WNODES; ii++)
	{
	  if (( wh = G.g_wlist[ii].w_id ) != 0)	/* yes, assignment!	*/
	  {
	    ...
	    /* WF_TOP emits the one required clipped WM_REDRAW. */
	    if (justtop)
	      return;
	  } /* if */
	} /* for */
...
*/
WORD win_isel(LPTREE olist, WORD root, WORD curr)
{
...
LPBYTE win_iname(WORD curr)
{
	...
	if (!pib)
	  return (LPBYTE) 0;
	ptext = (LPBYTE) gem_near_words_pointer(pib->ib_ptext);
	return( ptext );
}
...
VOID win_sinfo(WNODE *pwin)
{
	...
	pn = pwin->w_path;
	lstlcpy(ADDR(G.g_1text), ini_str(STINFOST), sizeof(G.g_1text));
