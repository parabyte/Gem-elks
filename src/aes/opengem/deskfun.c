/*	DESKFUN.C	08/30/84 - 05/30/85		Lee Lorenzen	*/
/*			10/2/86  - 01/16/87		MDF		*/
/*	merge source	5/27/87  - 5/28/87		mdf		*/
/*	for 2.3		6/11/87				mdf		*/

/*
*       Copyright 1999, Caldera Thin Clients, Inc.                      
*       This software is licenced under the GNU Public License.         
*       Please see LICENSE.TXT for further information.                 
*                                                                       
*                  Historical Copyright                                 
*	-------------------------------------------------------------
*	GEM Desktop					  Version 2.3
*	Serial No.  XXXX-0000-654321		  All Rights Reserved
*	Copyright (C) 1985 - 1987		Digital Research Inc.
*	-------------------------------------------------------------
*/

#include "ppddesk.h"

#if defined(ELKS) && ELKS
/*
 * DESKTOP.RSC remains the original asset.  The loaded free-string table can
 * therefore keep its historical text while this cold far-text table supplies
 * POSIX wording at the only point where Desktop turns those strings into a
 * visible alert.  It is read-only at runtime, uses exact offset:segment
 * pointers, and requires neither allocation nor resource conversion.
 */
#define ELKS_FAR_ALERT __attribute__((section(".fartext.s.elks_alert")))
MLOCAL BYTE FAR elks_alert_no_windows[] ELKS_FAR_ALERT =
	"[1][The GEM Desktop has no more available|"
	"windows. Close a window before opening|"
	"another folder.][ OK ]";
MLOCAL BYTE FAR elks_alert_trash_open[] ELKS_FAR_ALERT =
	"[1][Trash cannot be opened as a window.|"
	"Drag folders, documents, or applications|"
	"to Trash to delete them.][ OK ]";
MLOCAL BYTE FAR elks_alert_format[] ELKS_FAR_ALERT =
	"[3][Formatting will erase all information|"
	"on the selected floppy device.|Continue?][ OK |Cancel]";
MLOCAL BYTE FAR elks_alert_delete_filesystem[] ELKS_FAR_ALERT =
	"[3][Delete all information on the selected|"
	"filesystem? This cannot be undone.][ OK |Cancel]";
MLOCAL BYTE FAR elks_alert_desktop_target[] ELKS_FAR_ALERT =
	"[1][Folders, documents, and applications|"
	"stay inside the POSIX filesystem.|"
	"Copy them to a folder or move them|"
	"to Trash.][ OK ]";
MLOCAL BYTE FAR elks_alert_no_space[] ELKS_FAR_ALERT =
	"[1][The destination filesystem does not have|"
	"enough free space for this operation.][ OK ]";
MLOCAL BYTE FAR elks_alert_assets[] ELKS_FAR_ALERT =
	"[3][The Desktop cannot find DESKLO.ICN,|"
	"DESKHI.ICN, or DESKTOP.INF in the|"
	"configured POSIX search path.][Cancel]";
MLOCAL BYTE FAR elks_alert_save_config[] ELKS_FAR_ALERT =
	"[3][The Desktop configuration could not be|"
	"saved to /etc/DESKTOP.INF.][Retry|Cancel]";
MLOCAL BYTE FAR elks_alert_exit[] ELKS_FAR_ALERT =
	"[1][This will close the GEM Desktop and all|"
	"running applications.][ OK |Cancel]";
MLOCAL BYTE FAR elks_alert_root_trash[] ELKS_FAR_ALERT =
	"[1][The POSIX root filesystem cannot be|"
	"moved to Trash.][ OK ]";
MLOCAL BYTE FAR elks_alert_root_format[] ELKS_FAR_ALERT =
	"[1][The POSIX root is a mounted filesystem,|"
	"not a removable floppy device.][ OK ]";

MLOCAL GEM_SLOT_BYTE_POINTER
fun_elks_alert(WORD stnum)
{
	switch (stnum)
	{
	case STNOWIND:
		return elks_alert_no_windows;
	case STNOOPEN:
		return elks_alert_trash_open;
	case STFORMAT:
		return elks_alert_format;
	case STDELDIS:
		return elks_alert_delete_filesystem;
	case STNODRA1:
		return elks_alert_desktop_target;
	case STDISKFU:
		return elks_alert_no_space;
	case STNOFILE:
		return elks_alert_assets;
	case STNOINF:
		return elks_alert_save_config;
	case STEXTDSK:
		return elks_alert_exit;
	default:
		return (GEM_SLOT_BYTE_POINTER) 0;
	}
}
#endif

/*
*	Routine to transfer a string that requires integrated variables
*	that are merged in.  The resultant alert is then displayed;
*/
WORD fun_alert(WORD defbut, WORD stnum, ...)
{
	va_list ap;
	GEM_SLOT_BYTE_POINTER elks_alert;

	va_start(ap, stnum);

#if defined(ELKS) && ELKS
	elks_alert = fun_elks_alert(stnum);
	if (elks_alert)
		lstlcpy(&G.g_2text[0], elks_alert, sizeof(G.g_2text));
	else
	{
#else
	elks_alert = (GEM_SLOT_BYTE_POINTER) 0;
	(void) elks_alert;
	{
#endif
	rsrc_gaddr(R_STRING, stnum, (LPVOID *)&G.a_alert);
	/* Keep the original RSC string in place; only the merged alert is local. */
	lstlcpy(&G.g_2text[0], G.a_alert, sizeof(G.g_2text));
	}
	merge_v(&G.g_1text[0], &G.g_2text[0], ap);

	va_end(ap);
	return( form_alert(defbut, &G.g_1text[0]) );
}


WORD fun_root_delete_alert(VOID)
{
#if defined(ELKS) && ELKS
	lstlcpy(&G.g_1text[0], elks_alert_root_trash, sizeof(G.g_1text));
	return form_alert(1, &G.g_1text[0]);
#else
	return FALSE;
#endif
}


WORD fun_root_format_alert(VOID)
{
#if defined(ELKS) && ELKS
	lstlcpy(&G.g_1text[0], elks_alert_root_format, sizeof(G.g_1text));
	return form_alert(1, &G.g_1text[0]);
#else
	return FALSE;
#endif
}


VOID fun_msg(WORD type, WORD w3, WORD w4, WORD w5, 
			 WORD w6, WORD w7)
{
	/* keep DESKTOP messages internal to DESKTOP -- no AES call	*/
	G.g_rmsg[0] = type;
	G.g_rmsg[1] = gl_apid;
	G.g_rmsg[2] = 0;
	G.g_rmsg[3] = w3;
	G.g_rmsg[4] = w4;
	G.g_rmsg[5] = w5;
	G.g_rmsg[6] = w6;
	G.g_rmsg[7] = w7;
	hndl_msg();
}

/*
*	Rebuild window path and pflist
*/
VOID fun_rebld(WNODE *pwin)
{
	WORD		i, x, y, w, h;
	BYTE		*ptst;

	graf_mouse(HGLASS, 0);
						/* set up path to check	*/
						/*   against all windows*/
	ptst = &pwin->w_path->p_spec[0];
						/* check all wnodes	*/
	for(i = NUM_WNODES; i; i--)
	{
	  pwin = win_ith(i);
						/* if opened and same	*/
						/*   path then rebuild	*/
	  if ( (pwin->w_id) && (!strcmp(&pwin->w_path->p_spec[0], ptst)) )
	  {
	    pn_active(pwin->w_path);
	    desk_verify(pwin->w_id, TRUE);
// DESKTOP v1.2: Uncommented the "info line" bit.
	    win_sinfo(pwin);
	    wind_setl(pwin->w_id, WF_INFO, ADDR(&pwin->w_info[0]));

	    wind_get(pwin->w_id, WF_WXYWH, &x, &y, &w, &h);
	    fun_msg(WM_REDRAW, pwin->w_id, x, y, w, h);
	  } /* if */
	} /* for */
	graf_mouse(ARROW, 0);
} /* fun_rebld */

/*
*	Routine that creates a new directory in the specified window/path
*/
WORD fun_mkdir(WNODE *pw_node)
{
	PNODE		*pp_node;
	LPTREE		tree;
	WORD		more, cont, created, error, i;
	BYTE		fnew_name[12], unew_name[13], *ptmp;

	tree = G.a_trees[ADMKDBOX];
	pp_node = pw_node->w_path;
	ptmp = &G.g_srcpth[0];
	strlcpy(ptmp, pp_node->p_spec, sizeof(G.g_srcpth));
	i = 0;
	while (*ptmp)
	{
	  if (*ptmp == '/')
	    i++;
	  ptmp++;
	}
	if (i > MAX_LEVEL)
	{
	  fun_alert(1, STFO8DEE);
	  return(FALSE);
	}
	cont = TRUE;
	created = FALSE;
	while (cont)
	{
	  fnew_name[0] = 0;
	  inf_sset(tree, MKNAME, &fnew_name[0]);
	  show_hide(FMD_START, tree);
	  form_do(tree, 0);
	  cont = FALSE;
	  if (inf_what(tree, MKOK, MKCNCL))
	  {
	    inf_sget(tree, MKNAME, &fnew_name[0], sizeof(fnew_name));
	    unfmt_str(&fnew_name[0], &unew_name[0]);
	    if ( unew_name[0])
	    {
	      add_fname(&G.g_srcpth[0], &unew_name[0], sizeof(G.g_srcpth));
	      dos_mkdir(ADDR(&G.g_srcpth[0]));
	      if (DOS_ERR)
	      {
		/*
		 * EEXIST intentionally maps to E_NOACCESS because recursive
		 * Desktop copy uses that historical DOS result to merge folders.
		 * A read-only parent or permission failure has the same GEM value,
		 * so verify that this exact pathname now exists before displaying
		 * the original Retry dialog.  This is a cold, user-requested path;
		 * the extra stat-equivalent call is preferable to a false claim.
		 */
		error = DOS_AX;
		dos_chmod(ADDR(&G.g_srcpth[0]), F_GETMOD, 0);
		if (!DOS_ERR)
		  cont = fun_alert(2, STFOEXIS) - 1;
		else
		{
		  DOS_AX = error;
		  DOS_ERR = TRUE;
		  (void) d_errmsg();
		}
		del_fname(&G.g_srcpth[0]);
	      }
	      else
	      {
	        more = d_errmsg();
	        if (more)
		{
		  created = TRUE;
	          fun_rebld(pw_node);
		}
	      } /* else */
	    } /* if */
	  } /* if OK */
	} /* while */
	show_hide(FMD_FINISH, tree);
	return(created);
} /* fun_mkdir */

WORD fun_op(WORD op, PNODE *pspath, BYTE *pdest, 
			WORD dulx, WORD duly, WORD from_disk, WORD src_ob)
{
	WORD		fcnt, dcnt, more;
	GEM_U32_WORDS	size;
						/* do the operation	*/
	if (op != -1)
	{
	  if (op == OP_COPY)
	  {
	    if ( !par_chk(&pspath->p_spec[0], pspath->p_flist, 
				&G.g_tmppth[0]) )
	      return(FALSE);
	  }
						/* get count of source	*/
						/*   files		*/
	  more = dir_op(OP_COUNT, &pspath->p_spec[0], pspath->p_flist,
			pdest, &fcnt, &dcnt, &size, dulx, duly,
			from_disk, src_ob);
	  if (!more)
	    {
	      graf_mouse(ARROW, 0);
	      return(FALSE);
	    }
						/* do the operation	*/
	  if ( fcnt || dcnt )
	  {
	    more = dir_op(op, &pspath->p_spec[0], pspath->p_flist,
			  pdest, &fcnt, &dcnt, &size, dulx, duly,
			  from_disk, src_ob);
	    if (more)
	      return(TRUE);
	  } /* if */
	} /* if */
	graf_mouse(ARROW, 0);
	return(FALSE);
} /* fun_op */

/*
*	Routine to call when a list of files has been dragged on
*	top of a particular destination inside of a window.
*/
WORD fun_wdst(PNODE *pspath, BYTE *pdspec, WORD datype, FNODE *pdf, 
			WORD dulx, WORD duly, WORD from_disk, WORD src_ob, WORD *pdo_both)
//	PNODE		*pspath;		/* source path		*/
//	BYTE		*pdspec;		/* destination file spec*/
//	WORD		datype;			/* destination icon	*/
//	FNODE		*pdf;			/* destination file/fold*/
//	WORD		dulx, duly;		/* destination x, y	*/
//	WORD		from_disk;		/* TRUE if src is disk	*/
//	WORD		src_ob;			/* source object	*/
//	WORD		*pdo_both;		/* see AT_ISFOLD below  */
{
	WORD		op;
	BYTE		*pdname;
	LPICON		spib;
	WNODE		*pwname1, *pwname2;
	WORD		pdlen;
						/* default case: don't	*/
	*pdo_both = FALSE;			/* redraw both windows	*/
						/* set up destination	*/
						/*   path name		*/
	strlcpy(G.g_tmppth, pdspec, sizeof(G.g_tmppth));
	pdname = &G.g_tmppth[0];
	while (*pdname != '*')
	  pdname++;
	*pdname = 0;
						/* check destination	*/
	op = -1;
	pdlen = sizeof(G.g_tmppth) - (pdname - G.g_tmppth);
	switch( datype )
	{
	  case AT_ISFOLD:
						/* if destination is	*/
						/*   folder then append	*/
						/*   folder to window	*/
						/*   path, if it is a	*/
						/*   fake then treat it	*/
						/*   like open space	*/
		if (pdf->f_attr & F_FAKE)
		  strlcat(pdname, "*", pdlen);
		else
		{
		  strlcpy(pdname, pdf->f_name, pdlen);
		  strlcat(pdname, "/*", pdlen);
		}
		op = OP_COPY;
/* BugFix	*/
				/* check to see if this dest. folder	*/
				/*   is also an open window		*/
		pwname1 = &G.g_wlist[0];
		pwname2 = &G.g_wlist[1];
		if ( cmp_names(&G.g_tmppth[0], pwname1->w_name) ||
		     cmp_names(&G.g_tmppth[0], pwname2->w_name) )
		  *pdo_both = TRUE;
/* */
		break;
 	  case AT_ISDISK:
		strlcpy(G.g_tmppth, "/*", sizeof(G.g_tmppth));
		op = OP_COPY;
		break;
	  case AT_ISFILE:
						/* if destination is	*/
						/*   window or a file	*/
						/*   icon then use	*/
						/*   window path	*/
		strlcat(pdname, "*", pdlen);
		op = OP_COPY;
		break;
	  case AT_ISTRSH:
		op = OP_DELETE;
		break;
	} /* switch */
	return( fun_op(op, pspath, &G.g_tmppth[0], dulx, duly,
		from_disk, src_ob) );
} /* fun_wdst */

/*
*	Routine to call when the source of a drag is a disk
*	and the destination is either a window or another
*	disk.
*/
WORD fun_disk(WORD src_ob, WNODE *pdw, WORD datype, FNODE *pdf, WORD dulx, WORD duly)
{
	WORD		active, ret, do_both, source_is_app;
	FNODE		*pf;
	ANODE		*source_app;
	LPICON		spib;
	PNODE		*pspath;
	BYTE		chr;

	ret = FALSE;
	source_app = i_find(G.g_cwin, src_ob, &pf, &source_is_app);
#if defined(ELKS) && ELKS
	/* Every current AT_ISDISK node names "/", which must never be deleted. */
	if (datype == AT_ISTRSH && source_app
	    && source_app->a_type == AT_ISDISK)
	{
	  fun_root_delete_alert();
	  return FALSE;
	}
#endif
	if (G.g_iview == V_TEXT)
	{
	  /* Recover the same FNODE formerly carried in USERBLK.ub_parm. */
	  (void) source_app;
	  (void) source_is_app;
	  chr = pf ? pf->f_junk : 0;
	}
	else
	{
	  spib = get_spec(G.g_screen, src_ob);
	  chr = spib->ib_char;
	} /* else */
						/* build a source path	*/
	pspath = pn_open(chr, "", "*", "*", F_SUBDIR);
						/* if one available	*/
	if (pspath)
	{
						/* read the directory	*/
	  active = pn_active(pspath);
	  ret = FALSE;
	  if (active == E_NOFILES)
	    DOS_ERR = FALSE;
	  else
	  {
	    DOS_AX = active;
	    DOS_ERR = TRUE;
	    (void) d_errmsg();
	  }
						/* if files to copy	*/
	  if (active == E_NOFILES && pspath->p_flist)
	  {
						/* select all files	*/
	    for(pf = pspath->p_flist; pf; pf = pf->f_next)
	      pf->f_obid = 0;
	    G.g_screen[0].ob_state = SELECTED;
						/* do the copy or delete*/
	    if (pdw != (WNODE *) 0)
	      ret = fun_wdst(pspath, &pdw->w_path->p_spec[0], datype,
	      		     pdf, dulx, duly, TRUE, src_ob, &do_both);
	    else
	      ret = fun_op(OP_DELETE, pspath, &G.g_tmppth[0],
	      		   dulx, duly, TRUE, src_ob);
						/* return to normalcy	*/
	    G.g_screen[0].ob_state = NORMAL;
	  } /* if */
	  pn_close(pspath);
  		/* rebuild any windows with dpib.ib_char in title	*/
	  desk_clear(0);
	} /* if pspath */
	return(ret);
} /* fun_disk */

/*
*	Routine to call when several icons have been dragged from a
*	window to another window (it might be the same window) and 
*	dropped on a particular icon or open space.
*/
VOID fun_drag(WORD src_wh, WORD dst_wh, WORD dst_ob, WORD dulx, WORD duly)
{
	WORD		ret, junk, datype, src_ob, do_both;
	WNODE		*psw, *pdw;
	ANODE		*pda;
	FNODE		*pdf;

	psw = win_find(src_wh);
	pdw = win_find(dst_wh);
	if (pdw->w_path->p_spec[0] == '@' && dst_ob <= 3)
	  return;

	pda = i_find(dst_wh, dst_ob, &pdf, &junk);
	datype = (pda) ? pda->a_type : AT_ISFILE;

	if (psw->w_path->p_spec[0] != '@')
	{
	  ret = fun_wdst(psw->w_path, &pdw->w_path->p_spec[0], 
			 datype, pdf, dulx, duly, FALSE, 0, &do_both);
	  if (ret)
	  {
	    if (src_wh != dst_wh)
	      desk_clear(src_wh);
/* BugFix	*/
	    if (do_both)		/* force redraw of both windows	*/
	    {
	      fun_rebld(&G.g_wlist[0]);
	      fun_rebld(&G.g_wlist[1]);
	    }
/* */
	    else
	      fun_rebld(pdw);
	  } /* if ret */
	} /* if */
	else
	{
	  src_ob = 0;
	  while ( (src_ob = win_isel(G.g_screen, G.g_croot, src_ob)) != 0 )
	  {
	    ret = fun_disk(src_ob, pdw, datype, pdf, dulx, duly);
	    if (ret)
	      fun_rebld(pdw);
	  } /* while */
	} /* else */
} /* fun_drag */

/*
*	Routine to call when several icons have been dragged from a
*	desktop to the desktop and dropped on a particular icon.
*/
VOID fun_del(WNODE *pdw)
{
	WORD		src_ob, ret;
	LPICON		spib;
	BYTE		drvch[2], *ptmp;
#if defined(ELKS) && ELKS
	ANODE		*source;
	FNODE		*source_file;
	WORD		source_is_app;
#endif

	if (pdw->w_path->p_spec[0] != '@')
	{
	  ret = fun_op(OP_DELETE, pdw->w_path, &G.g_tmppth[0], 0, 0, 0, 0);
	  if (ret)
/*	    fun_rebld(pdw);*/
	    do_chkall(TRUE);
	}
	else
	{
	  src_ob = 0;
	  while ( (src_ob = win_isel(G.g_screen, G.g_croot, src_ob)) != 0 )
	  {
	    ret = 1;
#if defined(ELKS) && ELKS
	    source = i_find(G.g_cwin, src_ob, &source_file, &source_is_app);
	    (void) source_file;
	    (void) source_is_app;
	    if (source && source->a_type == AT_ISDISK)
	    {
	      fun_root_delete_alert();
	      continue;
	    }
#endif
/* BugFix	*/
	    if (G.g_iview == V_TEXT)
	    {
	      ptmp = &G.g_tmppth[0];
	      while (*ptmp < 0x40)
	        ptmp++;
	      drvch[0] = *ptmp;
	    } /* if V_TEXT */
/* */
	    else
	    {
	      spib = get_spec(G.g_screen, src_ob);
	      drvch[0] = (0x00FF & spib->ib_char);
	    } /* else */
	    drvch[1] = 0;
	    graf_mouse(ARROW, 0);
	    ret = fun_alert(2, STDELDIS, drvch);
	    graf_mouse(HGLASS, 0);
	    if (ret == 1)
	    {
	      fun_disk(src_ob, NULL, AT_ISTRSH, NULL, 0, 0);
	      do_chkall(TRUE);
	    }
	  } /* while */
	  graf_mouse(ARROW, 0);
	} /* else */
} /* fun_del */

WORD cmp_names(BYTE *psrc, BYTE *pdst)
{
	WORD		src_len, dst_len;
	BYTE		*psrc_end, *pdst_end;

	/*
	 * psrc ends in slash-star, while a window title ends in a slash.
	 * Compare only their directory portions without modifying either live
	 * path.  The comparison is deliberately case-sensitive, like POSIX.
	 */
	psrc_end = scan_slsh(psrc);
	pdst_end = pdst;
	while (*pdst_end)
	  pdst_end++;
	while (pdst_end > pdst && pdst_end[-1] == '/')
	  pdst_end--;

	src_len = 0;
	while (psrc + src_len < psrc_end)
	  src_len++;
	dst_len = 0;
	while (pdst + dst_len < pdst_end)
	  dst_len++;
	if (src_len != dst_len)
	  return(FALSE);
	while (src_len)
	{
	  if (*psrc++ != *pdst++)
	    return(FALSE);
	  src_len--;
	}
	return(TRUE);
} /* cmp_names */
