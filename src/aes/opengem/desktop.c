...
{
	desk_object_at(dst_tree, dst_icon)->ob_spec =
		desk_object_at(tree, icon)->ob_spec;
}
...
{
	graf_mouse( (turnon) ? HGLASS : ARROW, 0);
}
...
/*
 * men_update() owns menu items 18 through 44.  Retain their desired DISABLED
 * bits in two 16-bit masks while the original selection logic runs.  This
 * avoids the old enable-all/disable-subset sequence crossing INT EF twice for
 * an item whose final state did not change.  Bit zero represents the first
 * item in each range; shifts advance one place at a time for cheap 8088 code.
 */
static VOID
men_mark_state(UWORD *low_disabled, UWORD *high_disabled,
	WORD item, WORD enable)
{
	UWORD *disabled;
	UWORD bit;
	WORD distance;

	if (item >= OPENITEM && item <= PREFITEM) {
		disabled = low_disabled;
		distance = item - OPENITEM;
	} else if (item >= SAVEITEM && item <= TYPEITEM) {
		disabled = high_disabled;
		distance = item - SAVEITEM;
	} else
		return;
	bit = 1U;
	while (distance--)
...
static VOID
men_mark_list(UWORD *low_disabled, UWORD *high_disabled,
	BYTE *items, WORD enable)
{
	while (*items) {
		men_mark_state(low_disabled, high_disabled, *items, enable);
		items++;
	}
}

/* Emit only final state changes, preserving MENU_IENABLE redraw semantics. */
static VOID
men_commit_state(LPTREE tree, UWORD low_disabled, UWORD high_disabled)
{
	UWORD bit;
	UWORD desired;
	UWORD current;
	...
	for (item = OPENITEM; item <= PREFITEM; item++) {
		desired = low_disabled & bit;
		current = tree[item].ob_state & DISABLED;
		if (!!desired != !!current)
			menu_ienable(tree, item, !desired);
		bit <<= 1;
	}
	bit = 1U;
	for (item = SAVEITEM; item <= TYPEITEM; item++) {
		desired = high_disabled & bit;
		current = tree[item].ob_state & DISABLED;
		if (!!desired != !!current)
			menu_ienable(tree, item, !desired);
		bit <<= 1;
...
VOID men_update(LPTREE tree)
{
	...
	/* Items 18..33 begin enabled, exactly matching the former first loop. */
	low_disabled = 0;
	/* Items 34..44 retain their current state unless a list below changes it. */
	high_disabled = 0;
	bit = 1U;
	for (item = SAVEITEM; item <= TYPEITEM; item++) {
...
//  Not in DESKTOP v1.2
	...
	can_output = TRUE;
	  					/* disable some items	*/
	men_mark_list(&low_disabled, &high_disabled, ILL_ITEM, FALSE);
	     ...
	     nsel++)
	{
	  ...
	  switch (appl->a_type)
	  {
		...
		/* The sole root icon is a mounted filesystem, never a floppy. */
		pvalue = ILL_HDSK;
#else
	  ...
	  men_mark_list(&low_disabled, &high_disabled, pvalue, FALSE);
	} /* for */

	if (win_ontop()) pvalue = ILL_DESKTOP;	/* DESKTOP v1.2 */
	else			 pvalue = ILL_NOTOP;
	if (pvalue == ILL_DESKTOP)
		men_mark_list(&low_disabled, &high_disabled, pvalue, TRUE);
	else
		men_mark_list(&low_disabled, &high_disabled, pvalue, FALSE);
	...
	if (in_type)
		men_mark_state(&low_disabled, &high_disabled, TYPITEM, FALSE);
	men_commit_state(tree, low_disabled, high_disabled);
 ...
 *		cursor_init();  */
	...
	/* These next two only in DESKTOP v1.2 */
	  ...
	  case OUTPITEM:
			...
			/* slash path slash-star, file.ext list */
		G.g_tail[1] = 0;
		if (pw && curr)
		{
...
// END BUGFIX */
	...
	if (clicks == 1)
	{
	  pre_obj = gr_obfind(G.a_screen, G.g_croot, mx, my);
	  pre_selected = ((pre_obj != G.g_croot) &&
			  (pre_obj != NIL) &&
			  (desk_object_at(G.g_screen, pre_obj)->ob_state
			   & SELECTED));
...
/* Bring in the keyboard shortcuts which weren't in 1.2 */
	  case ALTA:	/* Options: Install App	*/
		...
		break;
	  case ALTC:	/* Options: open the POSIX command shell. */
		menu_tnormal(G.a_trees[ADMENU], OPTNMENU, FALSE);
...
/*	wind_update(BEG_UPDATE);*/
	do
	{
	  ...
	  if (!path[0])	/* No more path; show root */
	  {
	    strlcpy(pname, ini_str(STDSKDRV), LEN_ZPATH);
	    strlcpy(ppath, "/*", LEN_ZPATH);
	    at_drvs = TRUE;
	  }
	  else
...
/*	    *pend++ = SPACE;*/
	    *pend = 0;
	    					/* now fix path		*/
	    pend = ppath;
	    pend += (strlen(ppath) - 2);	/* skip trailing slash-star */
						/* Remove last name	*/
...
WORD hndl_msg()
{
	...
	switch( G.g_rmsg[0] )
	{
	  ...
	  case WM_ONTOP:
		gl_untop = TRUE;
		/*
		 * Original MULTIAPP w_setmen() selected desk_tree[pid] and
		 * menu_tree[pid] when a foreground application returned to
		 * Desktop.  ELKS keeps each application alive, so SETTINGS sends
		 * this same notification after closing its style-zero activation
		 * window.  Reinstall the existing relocated Desktop trees directly;
		 * no window, resource, or framebuffer copy crosses the process
		 * boundary.  The following WM_REDRAW retains the historical
		 * gl_untop menu-state refresh above.
		 */
		wind_set(0, WF_NEWDESK, FP_OFF(G.a_screen),
			 FP_SEG(G.a_screen), TRUE, FALSE);
		menu_bar(G.a_trees[ADMENU], TRUE);
		break;
	  case PR_FINISH:
		/* The ELKS process manager has already reaped and freed the slot. */
		break;
#endif
	  case WM_CLOSED:
...
/*		} */
	  ...
	  case WM_SIZED:	// Reinstated in DESKTOP v1.2
		...
		else
		{
			wind_get(G.g_rmsg[3],WF_WXYWH, &x, &y, &w, &h);
			pw = win_find(G.g_rmsg[3]);
			r_set((GRECT *)&desk_object_at(G.g_screen,
							pw->w_root)->ob_x,
			      x, y, w, h);
		}
		change = TRUE;
...
/* DESKTOP v1.2 version of this function doesn't assume 2 fixed windows */
	next_save = &G.g_cnxsave.win_save[0];
...
//	WSAVE		*pws;
	LPTREE		tree;
	GEM_SLOT_WORD_POINTER	plong[2];
	WORD		hsave[2];
	BYTE		docopyrt;
/* initialize libraries	*/
#if DEBUG
	remove("/tmp/gemapp.log");
#endif
...
/* DESKTOP v1.2 has no splash screen. But later desktops do. */
	...
	if (docopyrt)
	{
	  ...
	  hsave[0] = plong[0][0];
	  hsave[1] = plong[1][0];
	  plong[0][0] = desk_word_multiply(12, gl_hchar);
	  plong[1][0] = desk_word_multiply(7, gl_hchar);
...
// not in DESKTOP v1.2	rsrc_gaddr(R_STRING, STASICON, (LPVOID *)&ad_picon);
	...
	strlcpy(gl_pmstr, ini_str(STPM), sizeof(gl_pmstr));
						/* paint an initial GEM */
						/* menu while desktop.inf */
...
#if MULTIAPP
	/*
	 * GEM/XM queried a DOS channel map here and flattened segment:offset
	 * addresses so Desktop could carve application arenas itself.  ELKS owns
	 * every process data/stack segment, so no physical address is exposed or
	 * retained.  The original accessory discovery and menu flow continue to
	 * run; only their memory owner changes.
	 */
	gl_bootdr = 'C';
	gl_untop = 0;
	proc_shrink(DESKPID);
	iac_init();
	/* ELKS exposes no truthful free-process-memory total to user space. */
...
#endif
	/*
	 * POSIX root is a namespace boundary, not a removable-media probe.
	 * app_detect() is idempotent, so install it even when DESKTOP.INF came
	 * from the resident shell buffer instead of being copied from disk.
	 */
	app_detect();
						/* initialize windows	*/
	win_start();
