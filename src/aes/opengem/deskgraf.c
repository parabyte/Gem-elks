...
/*
 * The original Desktop and Pacific VDI bindings share one parameter-array
 * set.  Keep the original Desktop capacities where they were already larger,
 * and enlarge only the three short arrays needed by the complete binding
 * closure.  These are unscaled 16-bit GEM words.  Sharing them avoids a
 * second VDI parameter block in the Desktop's limited ELKS data segment.
 */
...
VOID gsx_pline(WORD offx, WORD offy, WORD cnt, WORD *pts)
{
	...
	/*
	 * This is the original GEMGRAF.C path: translate once into the shared
	 * VDI point array, then issue one polyline.  Desktop drag outlines contain
	 * only five or nine points.  The later chunk loop which had replaced this
	 * code allocated 160 hot stack bytes and always made a second zero-point
	 * gsx_xline() call, adding one needless VDI style trap for every outline
	 * drawn and erased while the mouse moved.
	 */
	for (i=0; i<cnt; i++)
	{
		j = i * 2;
...
*/
VOID gsx_fix(MFDB *pfd, GEM_SLOT_BYTE_POINTER theaddr, WORD wb, WORD h)
{
	if (theaddr == ORGADDR)
	{
	  pfd->fwp = gl_workout[0] + 1;
	  pfd->fww = (WORD)desk_uword_divide((UWORD)pfd->fwp, 16, NULL);
	  pfd->fh  = gl_workout[1] + 1;
	  pfd->np  = gl_nplanes;
	}
	else
	{
	  pfd->fww = desk_word_divide(wb, 2, NULL);
	  pfd->fwp = desk_word_multiply(wb, 8);
	  pfd->fh  = h;
	  pfd->np  = 1;
	}
	pfd->ff = FALSE;
	/* Preserve the original MFDB offset and segment as two exact words. */
	pfd->mp = gem_near_pointer_words(theaddr);
}

/*
*	Routine to blit, to and from a specific area
*/
VOID gsx_blt(GEM_SLOT_BYTE_POINTER saddr, UWORD sx, UWORD sy, UWORD swb,
			 GEM_SLOT_BYTE_POINTER daddr, UWORD dx, UWORD dy, UWORD dwb,
...
*	form.
*/
VOID gsx_trans(GEM_SLOT_BYTE_POINTER saddr, UWORD swb,
	       GEM_SLOT_BYTE_POINTER daddr, UWORD dwb, UWORD h)
{
	gsx_fix(&gl_src, saddr, swb, h);
	gl_src.ff = TRUE;
	gl_src.np = 1;
...
VOID gsx_start()
{
	...
	vst_height( gl_handle, char_height, &gl_wptschar, &gl_hptschar, 
				&gl_wchar, &gl_hchar );
	gl_ncols = desk_word_divide(gl_width, gl_wchar, NULL);
	gl_nrows = desk_word_divide(gl_height, gl_hchar, NULL);
	gl_hbox = gl_hchar + 3;
	gl_wbox = x_mul_div(gl_hbox, gl_workout[3], gl_workout[4]);
	...
	r_set(&gl_rcenter,
		desk_word_divide(gl_width - gl_wbox, 2, NULL),
		desk_word_divide(gl_height - (gl_hbox + gl_hbox), 2, NULL),
			gl_wbox, gl_hbox);
