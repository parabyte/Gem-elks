...
VOID lstlcpy(GEM_SLOT_BYTE_POINTER dest, GEM_SLOT_BYTE_POINTER src,
	     int maxlen);
...
WORD dir_op(WORD op, BYTE *psrc_path, FNODE *pflist, BYTE *pdst_path, 
			WORD *pfcnt, WORD *pdcnt, GEM_U32_WORDS *psize,
...
/* deskinf.c */
WORD ob_sfcb(LPBYTE psfcb, BYTE *pfmt);
...
/*
 * ELKS/8086 arithmetic and object-address helpers.
 *
 * The original Desktop sources assumed that compiler-generated multiply and
 * divide instructions were acceptable.  They are very slow on a 4.77 MHz
 * 8088.  These helpers preserve the original 16-bit wrap and truncation rules
 * with one-bit shifts, additions and restoring subtraction only.  The byte
 * indexing helper returns a near pointer and wraps its offset at 16 bits, just
 * like the Desktop's original small data segment.
 */
UWORD desk_uword_multiply(UWORD left, UWORD right);
WORD desk_word_multiply(WORD left, WORD right);
UWORD desk_uword_divide(UWORD numerator, UWORD denominator,
			UWORD *remainder);
WORD desk_word_divide(WORD numerator, WORD denominator, WORD *remainder);
BYTE *desk_byte_index(BYTE *base, UWORD index, UWORD record_size);
LPOBJ desk_object_at(LPTREE tree, WORD obj);
...
/* Direct synchronous ELKS /bin/mkfs seam for original Format Disk. */
WORD gemdesk_posix_format(WORD drive);
