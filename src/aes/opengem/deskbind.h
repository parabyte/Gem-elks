...
/* */
	 ...
	 /*
	  * File and Desktop icon code indexes this table by the absolute
	  * g_screen OBJECT id, not by an item-pool-relative number.  The legal
	  * range therefore includes the window roots and the final NUM_SOBS-1
	  * item.  The former NUM_WOBS bound was five words short and a rotating
	  * next-fit allocator could expose that old overrun during normal redraws.
	  */
	 WORD		g_index[NUM_SOBS];
	 ...
	 GEM_U32_WORDS	g_nfiles;
	 GEM_U32_WORDS	g_ndirs;
	 GEM_U32_WORDS	g_size;
