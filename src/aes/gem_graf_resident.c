/*
 * gem_graf_resident.c - resident AES graphics interactions
 *
 * handles the graf box/drag/rubber/slide tracking, a call holding the mouse
 * button stays tied to its owner slot while the owner feeds it input samples
 */
...
/* twelve 64-byte PD records */
typedef BYTE GEM_GRAF_PD_MUST_BE_64_BYTES[(sizeof(GEM_GRAF_PD) == 64) ? 1 : -1];
...
/* on ia16 a far data pointer is the offset:segment pair */
...
{
	/* logical white is EGA 15, on mono anything nonzero is white */
...
/*
 * reject a rect whose edges could overflow a signed word, width/height can
 * run off screen and just get clipped
 */
static WORD
...
static WORD
gem_graf_rectangles_equal(const GEM_VDI_RECT *left, const GEM_VDI_RECT *right)
{
	return left->x == right->x && left->y == right->y
		&& left->width == right->width && left->height == right->height;
}

/* draw or erase up to two XOR outlines under one cursor hide and flush */
static WORD
...
/* unsigned word divide by shift-and-subtract */
static UWORD
...
/* work out step count and x/y step from the travel distance */
static VOID
	...
	UWORD *x_step, UWORD *y_step)
{
	...
	/* floor((x+y)/2) without overflowing the sum */
...
/*
 * (value * 1000) / divisor for the slider position, done in two-word math and
 * capped at 1000
 */
static UWORD
...
static VOID
gem_graf_animation_rect(GEM_VDI_SCREEN *screen, const GEM_VDI_RECT *rectangle)
{
...
/* draw the whole move path then erase it */
static WORD
...
/*
 * walk the box outline twice so the second XOR pass erases it, if size is set
 * the box grows two steps a frame while moving
 */
static WORD
gem_graf_box_path(const GEM_VDI_RECT *start, UWORD count,
	WORD x_step, WORD y_step, WORD size)
{
	GEM_VDI_SCREEN *screen;
	GEM_VDI_RECT rectangle;
	UWORD frames;
	...
	while (pass < 2U) {
		...
		while (frames--) {
			...
			rectangle.y = (WORD) ((UWORD) rectangle.y
				- (UWORD) y_step);
			if (size) {
				rectangle.width = (WORD)
					((UWORD) rectangle.width
					+ (UWORD) width_step);
				rectangle.height = (WORD)
					((UWORD) rectangle.height
					+ (UWORD) height_step);
			}
		}
		gem_vdi_flush(screen);
...
gem_graf_growbox(const GEM_VDI_RECT *small, const GEM_VDI_RECT *target)
{
	...
	centered.x = center_x;
	centered.y = center_y;
	return gem_graf_box_path(&centered, count,
		(WORD) x_step, (WORD) y_step, TRUE);
}
...
gem_graf_shrinkbox(const GEM_VDI_RECT *small, const GEM_VDI_RECT *target)
{
	...
	if (!gem_graf_rectangle_valid(gem_vdi_resident_screen(), small)
		...
		|| !gem_graf_stepcalc(small, target, &center_x, &center_y,
			&count, &x_step, &y_step)
		|| !gem_graf_box_path(target, count,
			(WORD) (0U - x_step), (WORD) (0U - y_step), TRUE))
		return FALSE;
	centered = *small;
	centered.x = center_x;
	centered.y = center_y;
	...
	GEM_BINDINGS_POINTER_SLOT address, UWORD bytes)
{
	...
	if (call->resource
		...
		available =
			(UWORD) (call->resource->storage.bytes - address.lo);
		if (bytes > available)
			return (VOID GEM_GRAF_FAR *) 0;
		{
...
/* find one OBJECT in the tree and check it really sits there */
	...
	if (call->resource
		...
		while (remaining && offset != tree.lo) {
			if (!gem_graf_range(offset, GEM_GRAF_OBJECT_BYTES,
				...
				|| offset >
				(UWORD) (0xffffU - GEM_GRAF_OBJECT_BYTES))
				return (OBJECT GEM_GRAF_FAR *) 0;
...
gem_graf_finish_tracker(GEM_GRAF_PD *pd)
{
	...
	if (pd->opcode == GEM_GRAF_RUBBOX) {
		...
		pd->output_count = 1U;
		if (pd->vertical) {
			distance = (UWORD) (pd->rectangle.y - pd->constraint.y);
			span = (UWORD) (pd->constraint.height
				- pd->rectangle.height);
		} else {
			distance = (UWORD) (pd->rectangle.x - pd->constraint.x);
...
/* keep the drag rect inside its constraint box, size already checked to fit */
static WORD
...
gem_graf_resident_input(const GEM_GRAF_INPUT *input)
{
	...
	if (!screen || input->mouse_x < 0 || input->mouse_y < 0
		|| input->mouse_x >= screen->xres
		|| input->mouse_y >= screen->yres)
		return FALSE;
	...
	if (!gem_graf_pd_matches(pd, input->generation_lo, input->generation_hi)
		|| pd->state != GEM_GRAF_PD_WAITING)
		return TRUE;

	/* finish when the button comes up */
	...
	if (pd->opcode != GEM_GRAF_WATCHBOX) {
		if (!gem_graf_update_tracker(pd, input->mouse_x,
				input->mouse_y)) {
			if (pd->outline_visible) {
				gem_graf_xor_pair(&pd->rectangle, TRUE,
					(const GEM_VDI_RECT *) 0, FALSE);
	...
	GEM_GRAF_EFFECTS *effects, WORD *handled)
{
	...
	*handled = FALSE;
	...
	if (opcode != GEM_GRAF_RUBBOX && opcode != GEM_GRAF_DRAGBOX
		...
		&& opcode != GEM_XGRF_STEPCALC && opcode != GEM_XGRF_2BOX)
		return FALSE;
	...
	if ((opcode == GEM_GRAF_RUBBOX
		...
		|| (opcode == GEM_XGRF_STEPCALC
			&& !gem_graf_call_shape(call, 6U, 6U, 0U))
		|| (opcode == GEM_XGRF_2BOX
			&& !gem_graf_call_shape(call, 9U, 1U, 0U)))
	...
	*handled = TRUE;
	...
	/*
	 * two extra calls the Desktop uses: one works out the centred start
	 * point and step counts, the other walks one box outline path
	 */
	if (opcode == GEM_XGRF_STEPCALC) {
		UWORD count;
		UWORD x_step;
		UWORD y_step;
		WORD center_x;
		...
		second.x = (WORD) call->int_in[2];
		second.y = (WORD) call->int_in[3];
		second.width = (WORD) call->int_in[4];
		second.height = (WORD) call->int_in[5];
		if (!gem_graf_stepcalc(&first, &second, &center_x, &center_y,
				&count, &x_step, &y_step))
			return gem_graf_malformed(call, handled);
		call->int_out[0] = TRUE;
		call->int_out[1] = (UWORD) center_x;
		call->int_out[2] = (UWORD) center_y;
		call->int_out[3] = count;
		call->int_out[4] = x_step;
		call->int_out[5] = y_step;
		return TRUE;
	}
	if (opcode == GEM_XGRF_2BOX) {
		first.x = (WORD) call->int_in[5];
		first.y = (WORD) call->int_in[6];
		first.width = (WORD) call->int_in[7];
		first.height = (WORD) call->int_in[8];
		/*
		 * int_in[3] was a clip flag we dont use, only the grow flag in
		 * int_in[4] matters here
		 */
		result = gem_graf_box_path(&first, call->int_in[0],
			(WORD) call->int_in[1], (WORD) call->int_in[2],
			(WORD) call->int_in[4]);
	...
	if (opcode == GEM_GRAF_MBOX || opcode == GEM_GRAF_GROWBOX
		...
		if (opcode == GEM_GRAF_MBOX) {
			/* MBOX args: width, height, source x/y, destination x/y */
			first.width = (WORD) call->int_in[0];
			first.height = (WORD) call->int_in[1];
	...
	if (pd->state != GEM_GRAF_PD_FREE) {
		/* only one held request per channel */
		return gem_graf_malformed(call, handled);
	}
	...
	/* quick-out: button is already up so finish now */
...
WORD
gem_graf_resident_waiting(UWORD owner, UWORD generation_lo, UWORD generation_hi)
{
	GEM_GRAF_PD *pd;
