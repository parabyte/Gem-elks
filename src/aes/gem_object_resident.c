/*
 * gem_object_resident.c - resident AES object manager
 *
 * draws and manages the objects in a GEM tree - add, delete, draw, find,
 * offset, order and change. reads objects in place from the client DS or
 * the RSC segment and pushes bitmap rows to the VDI through a 64-word buffer
 */
...
/* the eight GEM fill densities, picked by the 3-bit pattern */
...
/* fixed buffers for formatting text */
...
/* how many whole rows fit in the 64-word scratch, index 0 unused */
...
/* on ia16 a far data pointer is just the offset:segment pair */
...
/* bytes left in one allowed segment */
static WORD
	...
	GEM_FAR_ADDRESS address, UWORD *available)
{
	...
	/* only synthetic calls inside the resident AES set this segment */
...
/* build the far pointer only after the range check, host branch is the
 * native smoke test, not built for ELKS */
...
/* add signed coords with plain word wrap */
static WORD
...
/* find the tree end (LASTOB), bounds-checked against the RSC or client
 * segment */
static WORD
gem_object_open_tree(GEM_OBJECT_CONTEXT *context)
{
	...
	if (context->resource
		...
		if (!header
			|| header->rsh_rssize !=
			context->resource->storage.bytes || !header->rsh_nobs
		...
		while (remaining && offset != context->tree.lo) {
			if (!gem_object_range(offset, GEM_OBJECT_BYTES,
					context->resource->storage.bytes)
				|| offset >
				(UWORD) (0xffffU - GEM_OBJECT_BYTES))
				return FALSE;
...
/* add up the x,y offset of an object from the root */
static WORD
...
/* find the previous sibling, bounded so a broken sibling cycle ends */
static WORD
...
/* count one sibling list, the tail child points back to the parent not
 * NIL */
static WORD
	...
	UWORD *count)
{
	...
	if (!count || parent < ROOT || (UWORD) parent >= context->object_count)
		return FALSE;
...
/* add a child to a parents sibling list, with bounds checks */
static WORD
gem_object_add_child(GEM_OBJECT_CONTEXT *context, WORD parent, WORD child)
{
	...
	/* an unused object has ob_next set to NIL */
...
/* unhook one object from its parents sibling list, fixing head or tail if
 * it was at either end. the object is left alone so it can be re-added later */
static WORD
gem_object_delete(GEM_OBJECT_CONTEXT *context, WORD deleted)
{
	OBJECT GEM_OBJECT_FAR *parent_object;
	OBJECT GEM_OBJECT_FAR *deleted_object;
	OBJECT GEM_OBJECT_FAR *object;
	WORD parent;
	WORD next;
	WORD previous;
	...
	if (deleted <= ROOT || (UWORD) deleted >= context->object_count)
		return FALSE;
	parent = gem_object_parent(context, deleted, &next, &valid);
	...
	parent_object = gem_object_at(context, (UWORD) parent);
	deleted_object = gem_object_at(context, (UWORD) deleted);
	previous = NIL;
	if (parent_object->ob_head == deleted) {
		if (parent_object->ob_tail == deleted) {
			next = NIL;
			parent_object->ob_tail = NIL;
		}
		parent_object->ob_head = next;
	} else {
		previous = gem_object_previous(context, parent, deleted);
		if (previous == NIL)
			return FALSE;
		object = gem_object_at(context, (UWORD) previous);
		object->ob_next = next;
	...
	deleted_object->ob_next = NIL;
	return TRUE;
}

/* move an object to a new spot in the sibling list, position 0 is the head
 * and NIL the tail, checked before anything is unlinked */
static WORD
gem_object_order(GEM_OBJECT_CONTEXT *context, WORD moved, WORD new_position)
{
	...
	/* unhook the object from its sibling list */
	previous = NIL;
	if (parent_object->ob_head == moved)
		parent_object->ob_head = next;
	else {
	...
	/* insert at head, tail, or after position - 1 */
	if (new_position == 0) {
...
/* read a 4-byte ob_spec, follow one INDIRECT hop if flagged */
static WORD
...
/* map GEM logical colors to the PC drivers EGA numbers. on a 2-color
 * adapter only logical WHITE stays lit so colored text stays readable */
static GEM_VDI_COLOR
gem_object_native_color(const GEM_OBJECT_CONTEXT *context, UWORD logical)
{
	if (context->screen && context->screen->colors <= 2U)
		return (logical & 15U) == GEM_OBJECT_LOGICAL_WHITE ? 15U : 0U;
	switch (logical & 15U) {
	case 0:
		return 15U;
	case 1:
...
/* unpack the fields of a packed box color word */
static VOID
...
/* draw a border, thickness inward or outward */
static WORD
	...
	const GEM_VDI_RECT *rectangle, WORD thickness, UWORD color)
{
	...
	gem_vdi_set_foreground(context->screen,
		gem_object_native_color(context, color));
	if (thickness < 0)
		thickness--;
	do {
	...
	const GEM_VDI_RECT *rectangle, UWORD color, UWORD pattern)
{
	if (rectangle->width <= 0 || rectangle->height <= 0)
		return FALSE;
	gem_vdi_set_mode(GEM_VDI_REPLACE);
	/* IP_HOLLOW clears to white in replace mode */
	if (pattern == GEM_OBJECT_IP_HOLLOW) {
		gem_vdi_set_foreground(context->screen,
			gem_object_native_color(context,
				GEM_OBJECT_LOGICAL_WHITE));
	...
	gem_vdi_set_background(context->screen,
		gem_object_native_color(context, GEM_OBJECT_LOGICAL_WHITE));
...
gem_object_string(const GEM_OBJECT_CONTEXT *context,
	GEM_FAR_ADDRESS address, UWORD maximum, UWORD *length, UWORD *underline)
{
...
/* wrap a near AES formatting buffer in a far pointer slot */
...
/* 80 cell cap so the sum dont wrap */
static WORD
gem_object_text_pixels(const GEM_OBJECT_CONTEXT *context, UWORD characters)
{
	...
	pixels = 0;
	while (characters) {
		pixels = gem_object_add(pixels,
			(WORD) context->character_width);
		characters--;
	}
	return pixels;
}
	...
	WORD replace)
{
	...
	/* the replace bit means opaque text, so paint the extent white first */
	if (replace) {
		gem_vdi_set_mode(GEM_VDI_REPLACE);
		gem_vdi_set_foreground(context->screen,
			gem_object_native_color(context,
				GEM_OBJECT_LOGICAL_WHITE));
		gem_vdi_fill_rect(context->screen, x, y, width,
			(WORD) context->character_height);
	}
	...
	/* draw the underscore overlay without copying the text */
	prefix = underline + 1U;
...
/* merge the raw text into the template into the formatting buffer */
static WORD
	...
	*border_color = border;
	...
	if ((object_type == G_BOXTEXT || object_type == G_FBOXTEXT)
		&& (!gem_object_border(context, rectangle, *thickness, border)
			|| (*thickness > 0
				&& !gem_object_inside_rectangle(&interior,
					*thickness))
			|| !gem_object_fill(context, &interior, inside,
				pattern)))
		return FALSE;
	...
	return gem_object_text(context, object_type, string, length, underline,
		&interior, ted.te_just, text, replace);
}

/* copy 1-bit rows through the 64-word scratch in whole-row groups, the VDI
 * wants a near source pointer. anything wider than 128 pixels goes out in
 * 128-pixel column strips, each starting 16 source bytes after the last */
static WORD
	...
	UWORD height, UWORD foreground, UWORD background, WORD transparent)
{
	...
	if (!width || !height || width > (UWORD) (0xffffU - 15U))
		return FALSE;
	row_bytes = (UWORD) (((width + 15U) >> 4) << 1);
	gem_vdi_set_mode(GEM_VDI_REPLACE);
	gem_vdi_set_foreground(context->screen,
		gem_object_native_color(context, foreground));
	gem_vdi_set_background(context->screen,
		gem_object_native_color(context, background));
	gem_vdi_set_use_background(!transparent);
	width_left = width;
	while (width_left) {
		strip_width = width_left;
		if (strip_width > GEM_OBJECT_BITMAP_WIDTH)
			strip_width = GEM_OBJECT_BITMAP_WIDTH;
		words_per_row = (UWORD) ((strip_width + 15U) >> 4);
		if (!words_per_row
			|| words_per_row > GEM_OBJECT_BITMAP_ROW_WORDS)
			return FALSE;
		strip_bytes = (UWORD) (words_per_row << 1);
		rows_per_group = gem_object_bitmap_rows[words_per_row];
		row_address = source_address;
		row = 0;
		remaining = height;
		while (remaining) {
			...
			while (copied_rows < group_rows) {
				source = (UWORD GEM_OBJECT_FAR *)
					gem_object_pointer(context, row_address,
					strip_bytes);
				if (!source)
					return FALSE;
				word = 0;
				while (word < words_per_row) {
					gem_object_bitmap_row[buffer_word] =
						source[word];
					buffer_word++;
					word++;
				}
				...
				if (remaining) {
					if (row_bytes
						...
						(UWORD) (0xffffU -
							row_address.lo))
						return FALSE;
					row_address.lo = (UWORD) (row_address.lo
						+ row_bytes);
				}
			}
			gem_vdi_bitmap(context->screen, x, gem_object_add(y,
					(WORD) row), (WORD) strip_width,
				(WORD) group_rows, gem_object_bitmap_row);
			row = (UWORD) (row + group_rows);
		}
		width_left = (UWORD) (width_left - strip_width);
		if (width_left) {
			if (16U > (UWORD) (0xffffU - source_address.lo))
				return FALSE;
			source_address.lo = (UWORD) (source_address.lo + 16U);
			x = gem_object_add(x, (WORD) GEM_OBJECT_BITMAP_WIDTH);
		}
	}
	return TRUE;
}
	...
	const GEM_VDI_RECT *rectangle)
{
	...
	/* 8190 cap keeps bi_wb times eight inside one 8086 word */
	if (image.bi_wb <= 0 || image.bi_wb > 8190 || (image.bi_wb & 1)
		|| image.bi_hl <= 0 || image.bi_x != 0 || image.bi_y < 0)
		return FALSE;
	width = (UWORD) image.bi_wb << 3;
	skip = (UWORD) image.bi_y;
	while (skip) {
		if ((UWORD) image.bi_wb > (UWORD) (0xffffU - image.bi_pdata.lo))
			return FALSE;
...
/* draw a mono icon, G_CICON falls back to mono */
static WORD
	...
	const GEM_VDI_RECT *object_rectangle, UWORD state)
{
	...
	if (!((state & WHITEBAK) && background == GEM_OBJECT_LOGICAL_WHITE)
		&& !gem_object_far_bitmap(context, icon.ib_pmask, image.x,
			image.y, (UWORD) image.width, (UWORD) image.height,
	...
	if (text_rectangle.width > 0 && text_rectangle.height > 0) {
		if (!gem_object_string(context, icon.ib_ptext,
			...
			|| !gem_object_text(context, G_ICON, icon.ib_ptext,
				length, GEM_OBJECT_NO_UNDERLINE,
				&text_rectangle, GEM_OBJECT_TE_CNTR,
				text_foreground, FALSE))
			return FALSE;
	}
	return TRUE;
}

/* DISABLED gray-out: knock every other pixel white through the transparent
 * bitmap path. row word is picked by the screen parity of its first pixel
 * so the checkerboard stays screen-aligned like the VDI fill patterns */
static WORD
gem_object_halftone(GEM_OBJECT_CONTEXT *context, const GEM_VDI_RECT *rectangle)
{
	UWORD words_per_row;
	UWORD rows_per_group;
	UWORD group_rows;
	...
	gem_vdi_set_foreground(context->screen,
		gem_object_native_color(context, GEM_OBJECT_LOGICAL_WHITE));
	gem_vdi_set_use_background(FALSE);
	x = rectangle->x;
	width_left = (UWORD) rectangle->width;
	while (width_left) {
		strip_width = width_left;
		if (strip_width > GEM_OBJECT_BITMAP_WIDTH)
			strip_width = GEM_OBJECT_BITMAP_WIDTH;
		...
		rows_per_group = gem_object_bitmap_rows[words_per_row];
		y = rectangle->y;
		remaining = (UWORD) rectangle->height;
		while (remaining) {
			group_rows = rows_per_group;
			if (group_rows > remaining)
				group_rows = remaining;
			...
			gem_vdi_bitmap(context->screen, x, y,
				(WORD) strip_width, (WORD) group_rows,
				gem_object_bitmap_row);
			y = gem_object_add(y, (WORD) group_rows);
			remaining = (UWORD) (remaining - group_rows);
		}
		x = gem_object_add(x, (WORD) GEM_OBJECT_BITMAP_WIDTH);
		width_left = (UWORD) (width_left - strip_width);
	}
...
/* draw the state overlays on top of the base object */
static WORD
	...
	UWORD border)
{
	...
	if ((state & SHADOWED) && thickness) {
		...
		gem_vdi_set_mode(GEM_VDI_REPLACE);
		/* the shadow takes the objects border color */
		gem_vdi_set_foreground(context->screen,
			gem_object_native_color(context, border));
		/* measure from the bordered interior, each strip is
		 * 2*th thick and starts th past the interior */
		gem_vdi_fill_rect(context->screen, interior.x,
			gem_object_add(interior.y,
				gem_object_add(interior.height, amount)),
			gem_object_add(interior.width, amount),
			gem_object_add(amount, amount));
		gem_vdi_fill_rect(context->screen,
			gem_object_add(interior.x,
				gem_object_add(interior.width, amount)),
			interior.y, gem_object_add(amount, amount),
			gem_object_add(interior.height,
				gem_object_add(amount,
					gem_object_add(amount, amount))));
	}
	if (state & CHECKED) {
		gem_vdi_set_mode(GEM_VDI_REPLACE);
		...
		gem_vdi_line(context->screen, gem_object_add(interior.x, 2),
			gem_object_add(interior.y, 7),
			gem_object_add(interior.x, 4),
			gem_object_add(interior.y, 9), TRUE);
		gem_vdi_line(context->screen, gem_object_add(interior.x, 4),
			gem_object_add(interior.y, 9),
			gem_object_add(interior.x, 8),
	...
	/* the gray-out comes before a SELECTED inversion */
	if ((state & DISABLED)
		&& !gem_object_halftone(context, &interior))
		return FALSE;
...
/* draw one object (no children), with bounds checks */
static WORD
	...
	WORD screen_x, WORD screen_y)
{
	...
	/* a zero-extent object is skipped not failed, DESKTOP.RSC has a
	 * zero-width G_STRING. a negative size is still malformed */
	if (!rectangle.width || !rectangle.height)
		return TRUE;
	if (rectangle.width < 0 || rectangle.height < 0)
		return FALSE;
	/* fixed types keep a black border, boxes and TEDINFOs crack theirs */
	border = GEM_OBJECT_LOGICAL_BLACK;
	thickness = 0;
	if (type == G_TITLE)
		thickness = 1;
		...
		if (type == G_BOXCHAR) {
			...
			if (gem_object_formatted[0]
				&& !gem_object_text(context, type,
					gem_object_near_slot
					(gem_object_formatted), 1U,
					GEM_OBJECT_NO_UNDERLINE, &interior,
					GEM_OBJECT_TE_CNTR, text, replace))
				return FALSE;
		}
		break;
	case G_TEXT:
	...
	case G_BUTTON:
		...
		if (!gem_object_fill(context, &interior,
			...
			|| !gem_object_border(context, &rectangle, thickness,
				GEM_OBJECT_LOGICAL_BLACK)
			|| !gem_object_string(context, spec,
				GEM_OBJECT_MAX_TEXT, &length, &underline)
			|| !gem_object_text(context, type, spec, length,
				underline, &rectangle, GEM_OBJECT_TE_CNTR,
				GEM_OBJECT_LOGICAL_BLACK, FALSE))
			return FALSE;
		break;
	case G_STRING:
	case G_TITLE:
		if (!gem_object_string(context, spec, GEM_OBJECT_MAX_TEXT,
				&length, &underline)
			|| !gem_object_text(context, object->ob_type, spec,
				length, underline, &rectangle,
				GEM_OBJECT_TE_LEFT, GEM_OBJECT_LOGICAL_BLACK,
				FALSE))
			return FALSE;
		break;
	case G_IMAGE:
	...
	case G_USERDEF:
	case G_DTMFDB:
		/* the resident AES cant run client draw code in its own task so
		 * the body stays empty, the state overlays still go out */
		break;
	default:
		return FALSE;
	}
	return gem_object_states(context, type, object->ob_state,
		object->ob_flags, &rectangle, thickness, border);
}

/* walk the whole tree drawing every object, flattened so it never recurses */
static WORD
...
/* find the object under a point, searches in reverse z order */
static WORD
	...
	UWORD depth, WORD mouse_x, WORD mouse_y)
{
	...
	if (current != ROOT) {
		...
		if (!valid)
			return NIL;
		if (parent != NIL
			&& !gem_object_offset(context, parent, &origin.x,
				&origin.y))
			return NIL;
	}
	done = FALSE;
	...
	while (!done && steps) {
		...
		if (gem_object_inside(mouse_x, mouse_y, &rectangle)
			...
			current =
				gem_object_previous(context, last_found,
				current);
			if (current == NIL)
				done = TRUE;
		} else {
	...
	WORD *handled)
{
	...
	*handled = FALSE;
	...
	if ((opcode == GEM_OBJECT_OBJC_ADD
		...
		|| (opcode == GEM_OBJECT_OBJC_DELETE
			&& !gem_object_call_shape(call, 1U, 1U, 1U))
		|| (opcode == GEM_OBJECT_OBJC_DRAW
	...
	*handled = TRUE;
	...
	if (opcode == GEM_OBJECT_OBJC_DELETE) {
		result = gem_object_delete(&context, (WORD) call->int_in[0]);
		if (!result)
			return gem_object_malformed(call, handled);
		call->int_out[0] = TRUE;
		return TRUE;
	...
	if (opcode == GEM_OBJECT_OBJC_OFFSET) {
		if (!gem_object_offset(&context, (WORD) call->int_in[0], &x,
				&y))
			return gem_object_malformed(call, handled);
		call->int_out[0] = TRUE;
	...
	/* store the new state first, then try the XOR fast path */
	...
	if (type != G_ICON && type != G_CICON && type != G_BUTTON
		...
		if (thickness > 0
			...
			gem_vdi_set_foreground(context.screen,
				gem_object_native_color(&context,
					GEM_OBJECT_LOGICAL_WHITE));
			gem_vdi_fill_rect(context.screen, rectangle.x,
