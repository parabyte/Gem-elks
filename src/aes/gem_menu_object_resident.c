/*
 * gem_menu_object_resident.c - resident AES menu bar renderer
 *
 * draws the menu bar and its objects straight out of the RSC segment, each
 * menu is tagged with the owner and generation of the app that put it up
 */
...
/* far pointer only built at the already checked RSC boundary */
...
/* caller already bounded index by object_count */
static OBJECT __far *
...
/* find a tree root on an exact OBJECT boundary and count its objects up to
 * LASTOB */
static WORD
	...
	GEM_FAR_ADDRESS tree, UWORD *object_count)
{
	...
	if (header->rsh_rssize != resource->storage.bytes
		...
		|| !gem_menu_range(header->rsh_object,
			GEM_MENU_OBJECT_BYTES, resource->storage.bytes))
		return FALSE;

	/* reject an address between records */
	...
	/* LASTOB marks the last object in a tree */
	object = (OBJECT __far *) gem_menu_pointer(tree);
	count = 0;
	while (remaining) {
		if (!gem_menu_range(offset, GEM_MENU_OBJECT_BYTES,
	...
	/* every local link must stay inside this tree */
	object = (OBJECT __far *) gem_menu_pointer(tree);
	index = 0;
	while (index < count) {
	...
	/* check the ROOT/BAR/ACTIVE/MENUS layout, the menus index comes from
	 * ROOT.ob_tail */
...
/* unpack a packed box color word */
static VOID
...
/* shrink a rectangle inward by thickness */
static WORD
...
/* draw a box border, thickness inward or outward */
static WORD
	...
	GEM_FAR_ADDRESS spec, const GRECT *rectangle, WORD thickness)
{
	...
	/* no backing bitmap so an IP_HOLLOW fill just paints solid white */
	if (pattern == GEM_MENU_IP_HOLLOW)
		native = GEM_MENU_NATIVE_WHITE;
	else if (pattern != GEM_MENU_IP_SOLID
...
/*
 * measure a menu string, reject one with no NUL or past the 80 byte limit, and
 * note the first '_' position (or 0xffff)
 */
static WORD
...
/* draw a G_TITLE/G_STRING, with the '_' underline overlay */
static WORD
	...
	const GRECT *rectangle, WORD thickness)
{
	...
	if (thickness > 0 && !gem_menu_inside(&interior, thickness))
		return FALSE;
	/* step in by the thickness, then invert */
...
/* draw one G_BOX/G_IBOX/G_TITLE object */
static WORD
	...
	WORD screen_x, WORD screen_y)
{
	...
	if (type == G_TITLE) {
		/* titles get a 1 pixel thickness */
		thickness = 1;
	} else {
...
/* find an objects parent, with a step limit so a bad tree cant loop */
static WORD
...
/* work out an objects absolute x/y by adding up its parents */
static WORD
...
/* walk and draw every object without recursing, 10 stack entries cover ROOT
 * plus MAX_DEPTH of 8 */
static WORD
	...
	UWORD depth)
{
	...
	if (!valid)
		return FALSE;
	if (parent != NIL) {
		if (!gem_menu_object_offset(context, parent, &start_x,
				&start_y))
			return FALSE;
	} else {
		start_x = 0;
...
/*
 * draw the 1 pixel black line under the menu bar, the bar itself is painted
 * opaque white so callers put this line back after touching the strip
 */
WORD
gem_menu_object_resident_bar_rule(WORD line_y)
{
	GEM_VDI_SCREEN *screen;

	screen = gem_vdi_resident_screen();
	if (!screen || line_y < 0 || line_y >= screen->yres)
		return FALSE;
	gem_vdi_set_clip(screen, 0, NULL);
	gem_vdi_hide_cursor(screen);
	gem_vdi_set_mode(GEM_VDI_REPLACE);
	gem_vdi_set_foreground(screen, GEM_MENU_NATIVE_BLACK);
	gem_vdi_line(screen, 0, line_y, screen->xres - 1, line_y, TRUE);
	...
	UWORD show, WORD owner, UWORD generation_lo, UWORD generation_hi)
{
	...
	if (!context.screen || !context.character_width
		...
		|| resource->metrics.screen_width !=
		(UWORD) context.screen->xres
		|| resource->metrics.screen_height !=
		(UWORD) context.screen->yres)
		return FALSE;

	/* stretch THEBAR to the right screen edge */
	bar = gem_menu_object_at(tree, GEM_MENU_THEBAR);
	if ((WORD) bar->ob_x < 0 || bar->ob_x >= resource->metrics.screen_width)
		return FALSE;
	bar->ob_width = (UWORD) (resource->metrics.screen_width - bar->ob_x);

	/* clip off, hide cursor, draw, show cursor */
	...
	result = gem_menu_object_draw(&context, GEM_MENU_THEBAR,
		GEM_MENU_MAX_DEPTH);
	if (result) {
		/* draw the black lower rule under the bar */
		if (context.character_height > 32765U)
			result = FALSE;
		else
			result = gem_menu_object_resident_bar_rule(
				(WORD) (context.character_height + 2U));
	}
