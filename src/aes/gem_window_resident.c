/*
 * gem_window_resident.c - window manager for the resident ELKS AES
 *
 * the window manager: opens/closes/moves/sizes windows and tracks the title
 * bar gadgets, draw and message work comes back to the owner as effects
...
/* the SHADOWED state bit */
#define GEM_WINDOW_SHADOWED             0x0020U
...
/* ob_spec words for the window frame gadgets */
...
/* scroll bars are solid white 1170h since we have no saved bitmap underneath */
#define GEM_WINDOW_BAR_SPEC_LO           0x1170U
#define GEM_WINDOW_BAR_SPEC_HI           0x0001U
/* slider track, black border, gray over white */
#define GEM_WINDOW_SLIDE_SPEC_LO         0x1111U
#define GEM_WINDOW_SLIDE_SPEC_HI         0x0001U
#define GEM_WINDOW_ELEV_SPEC_LO          0x110bU
#define GEM_WINDOW_ELEV_SPEC_HI          0x0001U

/*
 * no colour-category table here so this value is baked in: black border and
 * text, solid pattern, white interior
 */
#define GEM_WINDOW_MONO_OPAQUE_COLOR     0x1170U

/* gesture states while a title bar button is held */
...
/* unnamed window draws an empty title, never a null far string */
static UBYTE gem_window_empty_text;

/* data segment for the shared title/info text pointers */
static UWORD
...
/* intersect two rects, checking the edge sums for overflow */
static WORD
gem_window_rect_intersect(const GRECT *clip, GRECT *rectangle)
{
	...
	bottom = clip_bottom <
		rectangle_bottom ? clip_bottom : rectangle_bottom;
...
/* union of two on-screen rectangles */
static WORD
gem_window_rect_union(const GRECT *source, GRECT *destination)
{
	...
	if (!gem_window_rect_edges(source, &source_right, &source_bottom)
		|| !gem_window_rect_edges(destination, &destination_right,
			&destination_bottom))
		return FALSE;
	left = source->g_x < destination->g_x ? source->g_x : destination->g_x;
	top = source->g_y < destination->g_y ? source->g_y : destination->g_y;
...
/* work out the inner work rect from the outer frame rect */
static WORD
...
/* floor(value * multiplier / divisor) in word maths, callers keep value <= divisor and multiplier <= 1000 */
static UWORD
...
/* append a child to the sibling list */
static VOID
...
/* remove a window from the sibling list */
static WORD
...
/* save the window order so a failed geometry change can be undone */
static VOID
...
static VOID
gem_window_rect_free(GEM_WINDOW_RESIDENT *manager, GEM_WINDOW_ORECT *rectangle)
{
	if (!rectangle)
		return;
...
static VOID
gem_window_release_list(GEM_WINDOW_RESIDENT *manager, GEM_WINDOW_SLOT *slot)
{
	GEM_WINDOW_ORECT *rectangle;
...
gem_window_orect_append(GEM_WINDOW_RESIDENT *manager,
	GEM_WINDOW_ORECT **head, GEM_WINDOW_ORECT **tail, const GRECT *piece)
{
	GEM_WINDOW_ORECT *rectangle;
...
/* split a rect around an occluder: top and bottom keep the full width, left and right cover only the overlap */
static WORD
gem_window_break_one(GEM_WINDOW_RESIDENT *manager,
	GEM_WINDOW_ORECT **link, const GRECT *occluder)
{
...
/* run one occluder over every piece without revisiting the new ones */
static WORD
	...
	GEM_WINDOW_SLOT *slot, const GRECT *occluder)
{
	...
	while (*link) {
		...
		/* skip past the fresh pieces to the old successor */
		scan = *link;
		if (scan == successor)
			continue;
		while (scan) {
...
/* rebuild the visible-piece lists for every window, only after a change */
static WORD
...
/* geometry or z-order damage has to repaint the Desktop underneath first */
static VOID
gem_window_background_dirty(GEM_WINDOW_EFFECTS *effects, const GRECT *rectangle)
{
	if (!effects || !rectangle || rectangle->g_w <= 0
		|| rectangle->g_h <= 0)
		return;
...
/* the drop shadow reaches a couple px right of and below the window, so mark that extra area dirty too */
static VOID
gem_window_frame_dirty(const GEM_WINDOW_RESIDENT *manager,
	GEM_WINDOW_EFFECTS *effects, const GRECT *rectangle)
{
	GRECT grown;
	WORD edge;
	...
	if (!rectangle || rectangle->g_w <= 0 || rectangle->g_h <= 0)
		return;
	gem_window_rect_copy(rectangle, &grown);
	grown.g_w += 2;
	edge = (WORD) (manager->screen_width - grown.g_x);
	if (grown.g_w > edge)
		grown.g_w = edge;
	grown.g_h += 2;
	edge = (WORD) (manager->screen_height - grown.g_y);
	if (grown.g_h > edge)
		grown.g_h = edge;
	gem_window_background_dirty(effects, &grown);
}
...
/* clip to work and visible area, then send WM_REDRAW */
static VOID
gem_window_redraw(GEM_WINDOW_RESIDENT *manager, GEM_WINDOW_EFFECTS *effects)
{
	GEM_WINDOW_SLOT *slot;
	OBJECT *root;
	OBJECT *object;
...
/*
 * repaint after a menu or form closes, clipped to the root work area, since
 * we have no saved bitmap to restore
 */
WORD
...
gem_window_create(GEM_WINDOW_RESIDENT *manager, WORD owner,
	UWORD generation_lo, UWORD generation_hi, UWORD kind, const GRECT *full)
{
	GEM_WINDOW_SLOT *slot;
	OBJECT *object;
	GRECT work;
	...
	/*
	 * desk accessories create with a zero rect and pass the real geometry
	 * at open time, so only check a nonzero full rect
	 */
	...
	WORD handle, const GRECT *rectangle, GEM_WINDOW_EFFECTS *effects)
{
	...
	if (!slot || (slot->flags & GEM_WINDOW_VF_INTREE)
		|| !gem_window_public_rect(manager, slot->kind, rectangle,
			&work))
		return FALSE;
	object = gem_window_tree_at(manager, (UWORD) handle);
	...
	WORD handle, GEM_WINDOW_EFFECTS *effects)
{
	...
	(void) gem_window_rebuild_rectangles(manager);
	gem_window_frame_dirty(manager, effects, &old);
	gem_window_redraw(manager, effects);
	return TRUE;
}
	...
	WORD top, GEM_WINDOW_EFFECTS *effects)
{
	...
	if (!gem_window_rebuild_rectangles(manager)) {
		...
		gem_window_rect_copy(&old_work, &slot->work);
		if (top)
			gem_window_tree_restore_order(manager, old_next,
				old_head, old_tail, old_top);
		(void) gem_window_rebuild_rectangles(manager);
		return FALSE;
	}
	gem_window_frame_dirty(manager, effects, &old);
	...
	UWORD output_count)
{
	if (!call)
		return FALSE;
	/* wind calls take no addr_in, so only check the two counts */
	return gem_aes_call_counts(call->control, input_count, output_count, 0)
		&& gem_aes_call_arrays(call->int_in, input_count,
		call->int_out, output_count,
		(const GEM_BINDINGS_POINTER_SLOT *) 0, 0);
}
...
gem_window_resident_dispatch(GEM_WINDOW_RESIDENT *manager,
	const GEM_WINDOW_CALL *call, GEM_WINDOW_EFFECTS *effects, WORD *handled)
{
	GEM_WINDOW_SLOT *slot;
	GRECT rectangle;
	WORD handle;
	...
	UWORD generation_lo, UWORD generation_hi, GEM_WINDOW_EFFECTS *effects)
{
	...
	while (handle < GEM_WINDOW_OPEN_LIMIT) {
		if ((slot->flags & GEM_WINDOW_VF_INUSE)
			...
			if (slot->flags & GEM_WINDOW_VF_INTREE) {
				gem_window_object_rect(object, &rectangle);
				gem_window_frame_dirty(manager, effects,
					&rectangle);
...
gem_window_active_types(GEM_WINDOW_RESIDENT *manager)
{
	...
	/*
	 * the frame is an IBOX so it doesnt blank the clients work area when
	 * rebuilt
	 */
	object->ob_type = GEM_WINDOW_G_IBOX;
	...
	object->ob_type = GEM_WINDOW_G_BOX;
	object->ob_flags = GEM_WINDOW_USECOLORCAT;
	object->ob_spec.lo = GEM_WINDOW_SLIDE_SPEC_LO;
...
/*
 * build a scroll bar, slider values use the 0..1000 scale, the elevator is
 * never smaller than one box or longer than the track
 */
static VOID
...
gem_window_resident_build_active(GEM_WINDOW_RESIDENT *manager, WORD handle)
{
	...
	if (slot->name.lo || slot->name.hi) {
		...
		manager->active_name.te_ptext.lo =
			(UWORD) &gem_window_empty_text;
	...
	if (slot->info.lo || slot->info.hi) {
		...
		manager->active_info.te_ptext.lo =
			(UWORD) &gem_window_empty_text;
	...
	/* GEM/3 frames cast a black drop shadow right and below */
	(objects + GEM_WINDOW_W_BOX)->ob_state = GEM_WINDOW_SHADOWED;
	...
	if (vertical) {
		...
		bar.g_h = area.g_h + 2;
		gem_window_active_bar(manager, objects, slot, active, TRUE,
			&bar);
	}
	if (horizontal) {
		bar.g_x = 0;
		...
		gem_window_active_bar(manager, objects, slot, active, FALSE,
			&bar);
	}
	if (vertical && horizontal) {
		...
		if (active && (slot->kind & GEM_WINDOW_SIZER)) {
			...
			/* fill the inactive corner instead of leaving a hole */
			object->ob_flags = GEM_WINDOW_USECOLORCAT;
	...
	GEM_WINDOW_EFFECTS *effects)
{
	...
	if (message == GEM_WINDOW_WM_MOVED || message == GEM_WINDOW_WM_SIZED) {
		if (!rectangle || !gem_window_public_rect(manager, slot->kind,
				rectangle, &work))
			return FALSE;
		return gem_window_append_message(manager, effects, slot,
			message, handle, rectangle);
	}
	if (message == GEM_WINDOW_WM_ARROWED
		...
		if (message == GEM_WINDOW_WM_ARROWED
			...
			return FALSE;
		if (message != GEM_WINDOW_WM_ARROWED && rectangle->g_x > 1000)
			return FALSE;
...
/* smallest window size is seven boxes, clamped to the screen */
static WORD
...
/*
 * handle a title bar gesture without blocking, the state stays in resident
 * data until a later sample reports the button release
 */
WORD
	...
	const GEM_WINDOW_INPUT *input, GEM_WINDOW_EFFECTS *effects)
{
	...
	/* a press in client work or on the desktop is left alone until release */
	...
	if (manager->control_state == GEM_WINDOW_CONTROL_IDLE) {
		...
		/* a click anywhere in an inactive window asks its owner to top it */
		if (manager->top != handle) {
			manager->control_state = GEM_WINDOW_CONTROL_RELEASE;
			(void) gem_window_resident_control_message(manager,
				handle, GEM_WINDOW_WM_TOPPED, (const GRECT *) 0,
				effects);
			return TRUE;
		}

		/* the top windows work rect belongs to the application */
		...
		if (gadget == GEM_WINDOW_W_CLOSER) {
			...
			if (slot->kind & GEM_WINDOW_HOTCLOSE) {
				manager->control_state =
					GEM_WINDOW_CONTROL_RELEASE;
				(void) gem_window_resident_control_message
					(manager, handle, GEM_WINDOW_WM_CLOSED,
					(const GRECT *) 0, effects);
			}
			return TRUE;
		}
		...
		if (gadget == GEM_WINDOW_W_UPARROW)
			...
			objects =
				gem_window_resident_build_active(manager,
				handle);
			if (objects
				&& gem_window_active_absolute(objects,
					gadget ==
					GEM_WINDOW_W_VSLIDE ? GEM_WINDOW_W_VELEV
					: GEM_WINDOW_W_HELEV, &elevator)) {
		...
		if (action != GEM_WINDOW_NIL) {
			...
			(void) gem_window_resident_control_message(manager,
				handle, GEM_WINDOW_WM_ARROWED, &parameter,
				effects);
			return TRUE;
		}
		...
		if (gadget == GEM_WINDOW_W_VELEV
			...
			if (objects
				...
				&& gem_window_active_absolute(objects,
					gadget ==
					GEM_WINDOW_W_VELEV ? GEM_WINDOW_W_VSLIDE
					: GEM_WINDOW_W_HSLIDE,
	...
	/* while a gesture is live, every held-button sample belongs to it */
	if (button_down)
		return TRUE;
	...
	if (state == GEM_WINDOW_CONTROL_WATCH) {
		...
		if (!gem_window_add(rectangle.g_h, delta_y, &rectangle.g_h))
			rectangle.g_h =
				delta_y < 0 ? 1 : manager->screen_height;
		maximum = manager->screen_width - rectangle.g_x;
		minimum =
			gem_window_control_minimum(manager->box_width, maximum);
		rectangle.g_w =
			gem_window_control_clamp(rectangle.g_w, minimum,
			maximum);
		maximum = manager->screen_height - rectangle.g_y;
		minimum =
			gem_window_control_minimum(manager->box_height,
			maximum);
		rectangle.g_h =
			gem_window_control_clamp(rectangle.g_h, minimum,
			maximum);
		message = GEM_WINDOW_WM_SIZED;
		...
		if (span <= 0)
			...
			parameter.g_x =
				(WORD) gem_window_scale((UWORD) position, 1000U,
				(UWORD) span);
		}
	}
	...
	if (!message)
		return TRUE;
	if (message == GEM_WINDOW_WM_HSLID || message == GEM_WINDOW_WM_VSLID)
		(void) gem_window_resident_control_message(manager, handle,
			message, &parameter, effects);
	else
		(void) gem_window_resident_control_message(manager, handle,
			message, &rectangle, effects);
	return TRUE;
}
