/*
 * gem_startup_resident.c - GEM startup calls for resident ELKS AES
 *
 * the GEM startup calls: graphics handle, mouse cursor shape and
 * hide/show nesting, root-window size, the window update lock, and the
 * shell context. keeps only the fixed GEM state, mouse and client-memory
 * effects go to the outer owner
 */
...
/*
 * new generation on the same channel means the old client is gone,
 * so its abandoned update lock is released
 */
static GEM_STARTUP_PD *
...
/* check the XIF array counts before reading any indexed field */
static WORD
gem_startup_call_shape(const GEM_STARTUP_CALL *call, UWORD int_in_count,
	UWORD int_out_count, UWORD addr_in_count)
{
	return gem_aes_call_counts(call->control, int_in_count, int_out_count,
		addr_in_count)
		&& gem_aes_call_arrays(call->int_in, int_in_count,
		call->int_out, int_out_count, call->addr_in, addr_in_count);
}
...
/* a malformed but recognized request is an AES failure, not a fallthrough */
static WORD
...
/* unsigned-word wrap, then the signed GEM bit pattern */
static WORD
	...
	GEM_STARTUP_EFFECTS *effects, WORD *handled)
{
	...
	if (opcode == GEM_STARTUP_APPL_BVSET) {
		...
		if (!pd)
			return gem_startup_malformed(call, handled);
		/* APPL_BVSET shifts each 16-bit vector into the high half */
		gem_startup_bvdisk.lo = 0;
	...
	if (mode == GEM_STARTUP_APPL_BVEXT_SET) {
		/* addr_in carries plain values here, not pointers */
...
/* GRAF_MOUSE plus the cursor hide/show nesting rules */
static WORD
...
gem_startup_wind_get(const GEM_STARTUP_CALL *call, WORD *handled)
{
	...
	/* the root work area sits just below the menu bar */
	call->int_out[1] = 0;
...
/* WIND_UPDATE, the begin/end update lock */
static WORD
gem_startup_wind_update(const GEM_STARTUP_CALL *call, WORD *handled)
{
	...
	if (operation == GEM_STARTUP_BEG_UPDATE) {
		if (gem_startup_update_owner != GEM_STARTUP_OWNER_NONE
			...
			/* outer resident parks this call on its mutex FIFO */
			return FALSE;
		}
...
/* WIND_CALC, border to work area or back */
static WORD
gem_startup_wind_calc(const GEM_STARTUP_CALL *call, WORD *handled)
{
	...
	if ((wtype != GEM_STARTUP_WC_BORDER && wtype != GEM_STARTUP_WC_WORK)
		|| !gem_startup_screen_ready)
		return FALSE;
	pd = gem_startup_bind_pd(call->owner, call->generation_lo,
		call->generation_hi);
	...
	if (kind & (GEM_STARTUP_NAME | GEM_STARTUP_CLOSER | GEM_STARTUP_FULLER)) {
	...
	call->int_out[3] =
		(UWORD) gem_startup_word_sub(gem_startup_word_sub(width, left),
		right);
	call->int_out[4] =
		(UWORD) gem_startup_word_sub(gem_startup_word_sub(height, top),
		bottom);
	return gem_startup_finish(call, TRUE, handled);
}

/*
 * SHEL_GET initial context, before any SHEL_PUT the record is '#' then
 * zeros, done here as two fill effects
 */
static WORD
