/*
 * gem_menu_pull_resident.c - resident GEM menu state machine
 *
 * runs the pull-down menus: opening titles, tracking the mouse and keyboard,
 * and sending the pick back to the app. no blocking loop, the kernel runs
 * procs so gem_menu_pull_resident_input() steps the menu one input at a time
 */
...
static GEM_MENU_PULL_ACCESSORY gem_menu_accessories[GEM_MENU_PULL_ACCESSORIES];
static UWORD gem_menu_accessory_count;

/* far pointers only built from an offset and segment we already checked */
...
/* record offsets as sums of powers of two so ia16-gcc emits no MUL, callers
 * check their indexes first */
static UWORD
gem_menu_scale_24(UWORD value)
{
	...
	__asm__ volatile ("addw %1,%0":"+r" (result):"r"(eight):"cc");
	return result;
}
...
gem_menu_scale_28(UWORD value)
{
	...
	__asm__ volatile ("addw %0,%0":"+r" (result)::"cc");
	__asm__ volatile ("addw %1,%0":"+r" (result):"r"(eight):"cc");
	__asm__ volatile ("addw %1,%0":"+r" (result):"r"(four):"cc");
	return result;
}
...
gem_menu_scale_14(UWORD value)
{
	...
	result = eight;
	__asm__ volatile ("addw %1,%0":"+r" (result):"r"(four):"cc");
	__asm__ volatile ("addw %1,%0":"+r" (result):"r"(two):"cc");
	return result;
}
...
/* check one byte range without the sum wrapping */
static WORD
...
/* recheck the LASTOB extent, we keep the tree view across calls */
static WORD
...
/* walk at most object_count siblings so a broken cycle stops */
static WORD
	...
	WORD first, WORD declared_last, WORD *actual_last)
{
	...
	if (!gem_menu_link_valid(parent, tree->object_count)
		...
		|| first == GEM_MENU_PULL_NIL
		|| declared_last == GEM_MENU_PULL_NIL)
		return FALSE;
	current = first;
	remaining = tree->object_count;
	while (remaining--) {
...
/* check the ROOT/BAR/ACTIVE/MENUS layout */
static WORD
	...
	WORD *first_menu, WORD *last_menu)
{
	...
	while (index < tree->object_count) {
		...
		if (!gem_menu_link_valid(object->ob_next, tree->object_count)
			|| !gem_menu_link_valid(object->ob_head,
				tree->object_count)
			|| !gem_menu_link_valid(object->ob_tail,
				tree->object_count))
			return FALSE;
		index++;
	}
	object = gem_menu_object_at(tree, (UWORD) (tree->object_count - 1U));
	...
	*active_titles = object->ob_head;
	if (*active_titles < 0 || (UWORD) *active_titles >= tree->object_count)
		return FALSE;
...
/* coordinate adds wrap in one word */
static WORD
...
/* find a childs parent through the threaded sibling links */
static WORD
...
/* work out an objects absolute rectangle by adding up its parents */
static WORD
...
static WORD
gem_menu_point_inside(WORD x, WORD y, const GEM_MENU_PULL_RECTANGLE *rectangle)
{
	WORD right;
	WORD bottom;
	...
	const GEM_MENU_PULL_RECTANGLE *rectangle)
{
	...
	if (effects->draw_count >= GEM_MENU_PULL_DRAW_EFFECTS) {
		/* on overflow just redraw the whole bar */
		effects->draw_count = 0;
...
static WORD
gem_menu_desk_rectangle(WORD object_index, GEM_MENU_PULL_RECTANGLE *rectangle)
{
...
gem_menu_current_area_effect(GEM_MENU_PULL_EFFECTS *effects, UBYTE action)
{
	...
	/* on RESTORE_AREA grow the rect a bit so the outward G_BOX border
	 * doesnt leave pixels behind */
	if (action == GEM_MENU_PULL_RESTORE_AREA) {
		rectangle.x = (WORD) ((UWORD) rectangle.x - 1U);
		rectangle.y = (WORD) ((UWORD) rectangle.y - 1U);
		rectangle.width = (WORD) ((UWORD) rectangle.width + 5U);
		rectangle.height = (WORD) ((UWORD) rectangle.height + 6U);
	}
...
/* build the static M_DESK tree in fixed near storage */
static WORD
...
/* read ob_spec, follow one INDIRECT hop if flagged */
static WORD
...
/* letter after the first underscore */
static UBYTE
...
/* ALT_A through ALT_Z BIOS scan words */
...
/* copy a far RSC string into a near Desk snapshot buffer */
static VOID
...
/* titles and menu boxes sit in parallel sibling chains */
static WORD
...
/* hit-test only enabled children */
static WORD
...
gem_menu_current_item_state(WORD item)
{
	...
	return gem_menu_object_at(&gem_menu_pull.tree, (UWORD) item)->ob_state;
}
...
/* find the next or previous enabled item in a sibling list or M_DESK, with
 * wrap */
static WORD
gem_menu_list_find(UBYTE desk, WORD parent_index, WORD current, WORD direction)
{
	GEM_MENU_PULL_OBJECT GEM_MENU_PULL_FAR *far_parent;
	GEM_MENU_PULL_OBJECT GEM_MENU_PULL_FAR *far_object;
	GEM_MENU_PULL_OBJECT *near_parent;
...
/* match an underscore shortcut among the enabled children */
static WORD
gem_menu_main_shortcut(UBYTE letter)
{
	...
	while (item != GEM_MENU_PULL_NIL && remaining--) {
		...
		if (!(object->ob_state & GEM_MENU_PULL_DISABLED)
			&& gem_menu_shortcut_letter(&gem_menu_pull.tree,
				item) == letter)
			return item;
...
/* open one title: select it, save the area, draw the menu */
static WORD
	...
	GEM_MENU_PULL_EFFECTS *effects)
{
	...
	if (kind == GEM_MENU_PULL_KIND_DESK) {
		if (!gem_menu_build_desk()
			|| !gem_menu_desk_rectangle(GEM_MENU_PULL_ROOT,
				&rectangle))
			return FALSE;
...
/* close a pull-down, send MN_SELECTED or AC_OPEN if something got picked */
static VOID
gem_menu_close(WORD selected, GEM_MENU_PULL_EFFECTS *effects)
{
	...
	if (selected != GEM_MENU_PULL_NIL
		...
		/* an app selection stays selected until TNORMAL */
		title->ob_state = gem_menu_pull.title_base_state
			| GEM_MENU_PULL_SELECTED;
		main_item = selected;
		if (gem_menu_pull.current_kind == GEM_MENU_PULL_KIND_DESK) {
			/* the fake Information row maps back to the Desk item */
			GEM_MENU_PULL_OBJECT GEM_MENU_PULL_FAR *menu;
...
/* switch pull-downs: unselect the old one, restore, then open the new one */
static WORD
	...
	UBYTE keyboard)
{
	...
	if (!gem_menu_open_title(title, FALSE, effects))
		return FALSE;
	if (keyboard)
		gem_menu_set_current_item(gem_menu_current_item_find
			(GEM_MENU_PULL_NIL, TRUE), effects);
	return TRUE;
}

/* update the row selection from one mouse sample */
static VOID
...
gem_menu_keyboard(UWORD key, GEM_MENU_PULL_EFFECTS *effects)
{
	...
	case GEM_MENU_PULL_KEY_DOWN:
		item = gem_menu_current_item_find(gem_menu_pull.current_item,
			TRUE);
		gem_menu_set_current_item(item, effects);
		return TRUE;
...
/* copy text only within the target strings checked capacity */
static WORD
	...
	if (requested >= 0)
		...
		while (index < gem_menu_accessory_count) {
			...
			if (accessory->owner == call->owner
				&& accessory->generation_lo ==
				call->generation_lo
				&& accessory->generation_hi ==
				call->generation_hi)
				break;
			index++;
		}
	...
	UWORD output_count, UWORD address_count)
{
	if (!call)
		return FALSE;
	/* the call keeps tree and text in their own fields not addr_in, so
	 * just check the control[3] counts */
	...
	GEM_MENU_PULL_EFFECTS *effects)
{
	...
	if (!gem_menu_build_desk())
		return FALSE;
	/* repaint the whole strip, G_TITLE glyphs are transparent so a longer
	 * previous title would show through */
	if (effects)
		effects->redraw_all = TRUE;
	return TRUE;
}
	...
	GEM_MENU_PULL_EFFECTS *effects, WORD *handled)
{
	...
	*handled = TRUE;
	...
	if (opcode >= GEM_MENU_PULL_ICHECK && opcode <= GEM_MENU_PULL_TEXT) {
		...
		if (gem_menu_pull.tracking
			&& gem_menu_pull.current_kind ==
			GEM_MENU_PULL_KIND_DESK)
			gem_menu_close(GEM_MENU_PULL_NIL, effects);
	...
	UWORD generation_hi, GEM_MENU_PULL_EFFECTS *effects)
{
	...
	/* geometry mustnt change under an already saved Desk pull-down */
	changed = FALSE;
	index = 0;
	while (index < gem_menu_accessory_count) {
		accessory = gem_menu_accessory_at(index++);
	...
	GEM_MENU_PULL_DESK_OBJECT *snapshot)
{
	...
	if (object_index <= 2U) {
		...
		source = gem_menu_object_string(&gem_menu_pull.tree,
			object_index == 1U
			? menu->ob_head : information->ob_next, &available);
		if (!source)
			return FALSE;
